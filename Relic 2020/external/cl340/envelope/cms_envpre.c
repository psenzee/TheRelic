...
static int processKeyexchangeAction( INOUT ENVELOPE_INFO *envelopeInfoPtr,
	...
	if( preActionListPtr->action == ACTION_KEYEXCHANGE_PKC && \
		cryptStatusOK( krnlSendMessage( preActionListPtr->iCryptHandle,
										IMESSAGE_CHECK, NULL,
	...
	return( ( isDlpAlgo( cryptAlgo ) || isEccAlgo( cryptAlgo ) ) ? \
			OK_SPECIAL : CRYPT_OK );
	}
