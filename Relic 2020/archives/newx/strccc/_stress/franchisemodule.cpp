...
/*!
    ...
    $Header: //blaze/games/Madden/2010-NG/trunk/component/franchise/stress/franchisemodule.cpp#47 $
    $Change: 42730 $
    $DateTime: 2009/05/20 11:03:31 $
...
bool FranchiseModule::parseConfig(const ConfigMapBase& config)
{
    ...
    // This didn't work; connInfo = NULL.
    //const Blaze::ConfigMap* connInfo = config.getMap("connection-info");
    //if (connInfo != NULL)
    //{
    //    mNumConnections = connInfo->getUInt32("num-connections", 100);
    //}
...
bool FranchiseModule::initialize(const ConfigMapBase& config)
{
    ...
    if (mStatsEnabled)
    {
        StatsTables::GetInstance().ReadAll(1000);
    }
...
FranchiseInstance::~FranchiseInstance()
{
    ...
    // Delete all FranchiseIdData pointers in the mVecFranIdData vector.  They were allocated in findAllFranchiseIds.
    FranchiseIdDataVectorType::iterator iterFrans = mVecFranIdData.begin();
    for (; iterFrans != mVecFranIdData.end(); iterFrans++)
    {
...
BlazeRpcError FranchiseInstance::execute()
{
    ...
    if (interpreter && interpreter->GetState()) // if a state is created, then we're using lua
    {
        if (!mLuaThread)
        {
    ...
    case FranchiseModule::ACTION_INDEPTH_TEST:
        {
            ...
            for (int32_t index = 0; index < mCreateFranchiseCount; index++)
            {
                ...
                if (franId > 0 && !randomFranchise)
                {
                    ...
                    ////if (mOwner->isRpcAllowed (FranchiseModule::METRIC_ADVANCE_FRANCHISE)) { rpcResult = advanceFranchiseLocal (franId, FRANCHISE_STATE_DRAFT); }
                    ////if (mOwner->isRpcAllowed (FranchiseModule::METRIC_ADVANCE_FRANCHISE)) { rpcResult = advanceFranchiseLocal (franId, FRANCHISE_STATE_NONE); } // NONE cannot advance
...
{
    //StressConnection* connection = getConnection (); // save for later
...
BlazeRpcError FranchiseInstance::createFranchiseLocal (int32_t indexOfFranchise, FranchiseId* returnedCreatedFranId)
{
    ...
    if (rpcResult == ERR_OK)
    {
        ...
        fidData->seasonId         = 0;
        fidData->windowId         = 0;
        mVecFranIdData.push_back (fidData);
...
BlazeRpcError FranchiseInstance::deleteFranchiseLocal (FranchiseId franId)
{
    ...
    if (rpcResult == ERR_OK)
    {
        ...
        eastl::vector<FranchiseIdData*>::iterator iter = mVecFranIdData.begin();
        for (; iter != mVecFranIdData.end(); iter++)
        {
            if ((*iter)->fid == franId)
            {
...
BlazeRpcError FranchiseInstance::removeMemberLocal (FranchiseId franId)
{
    ...
    if (bzId) // it may be rare that we find such a member
    {
        ...
        {
            FranchiseIdDataVectorType::iterator iterFrans = mVecFranIdData.begin();
            FranchiseIdDataVectorType::iterator iterFransEnd = mVecFranIdData.end();
            for (; iterFrans != iterFransEnd; iterFrans++)
            {
                FranchiseIdData* data = *iterFrans;
                if (data && data->fid == franId)
                {
                    bool done = false;
                    while (!done)
                    {
...
void FranchiseInstance::fillFranchiseTeamStats(Franchise::SubmitFranchiseGameStatsRequest& req, Franchise::GetGameplayInformationResponse& ggiResp)
{
    ...
    if (statsTable && !statsTable->mRegularSchedule.empty())
    {
        const IntArray &stats = statsTable->mRegularSchedule[rand() % statsTable->mRegularSchedule.size()];
...
void FranchiseInstance::internalFillPlayerStats(Franchise::SubmitFranchiseGameStatsRequest& req, 
                                                ...
                                                int32_t randomAthleteCount1,
                                                int32_t randomAthleteCount2,
                                                eastl::vector<IntArray> &statArray)
{
    Franchise::SubmitFranchiseGameStatsRequest::Int32_tList* statsList = req.getStats();
...
void FranchiseInstance::fillInjuryPlayerStats(Franchise::SubmitFranchiseGameStatsRequest& req, Franchise::GetGameplayInformationResponse& ggiResp)
{
    ...
    StatsTables* statsTable = mOwner->getStatsTable();
    if (statsTable && !statsTable->mInjury.empty())
    {
        Franchise::SubmitFranchiseGameStatsRequest::Int32_tList* statsList = req.getStats();
        Franchise::SubmitFranchiseGameStatsRequest::Uint32_tList* entitiesList = req.getEntities();

        const IntArray &stats = statsTable->mInjury[0];
        ...
        for (; team1Iter !=  team1AthleteProperties->end(); team1Iter++)
        {
            ...
            const IntArray &stats1 = statsTable->mInjury[rand() % statsTable->mInjury.size()];
            for (int32_t i = 0; i< (int32_t)stats1.size(); i++)
            {
                statsList->push_back(stats1[i]);
            }
...
void FranchiseInstance::fillOffensivePlayerStat(Franchise::SubmitFranchiseGameStatsRequest& req, Franchise::GetGameplayInformationResponse& ggiResp)
{
    ...
    StatsTables* statsTable = mOwner->getStatsTable();
    if (statsTable && !statsTable->mOffensivePlayer.empty())
    {
        internalFillPlayerStats(req, ggiResp, randomAthleteCount1, randomAthleteCount2, statsTable->mOffensivePlayer);
    }
}
...
void FranchiseInstance::fillOffensiveLinePlayerStat(Franchise::SubmitFranchiseGameStatsRequest& req, Franchise::GetGameplayInformationResponse& ggiResp)
{
    ...
    StatsTables* statsTable = mOwner->getStatsTable();
    if (statsTable && !statsTable->mOffensiveLinePlayer.empty())
    {
        internalFillPlayerStats(req, ggiResp, randomAthleteCount1, randomAthleteCount2, statsTable->mOffensiveLinePlayer);
    }
}
...
void FranchiseInstance::fillDefensivePlayerStats(Franchise::SubmitFranchiseGameStatsRequest& req, Franchise::GetGameplayInformationResponse& ggiResp)
{
    ...
    StatsTables* statsTable = mOwner->getStatsTable();
    if (statsTable && !statsTable->mDefensivePlayer.empty())
    {
        internalFillPlayerStats(req, ggiResp, randomAthleteCount1, randomAthleteCount2, statsTable->mDefensivePlayer);
    }
}
void FranchiseInstance::fillKickPlayerStats(Franchise::SubmitFranchiseGameStatsRequest& req, Franchise::GetGameplayInformationResponse& ggiResp)
{
    ...
    StatsTables* statsTable = mOwner->getStatsTable();
    if (statsTable && !statsTable->mKickPlayer.empty())
    {
        internalFillPlayerStats(req, ggiResp, randomAthleteCount1, randomAthleteCount2, statsTable->mKickPlayer);
    }
}
void FranchiseInstance::fillKickReturnPlayerStats(Franchise::SubmitFranchiseGameStatsRequest& req, Franchise::GetGameplayInformationResponse& ggiResp)
{
    ...
    StatsTables* statsTable = mOwner->getStatsTable();
    if (statsTable && !statsTable->mKickReturnPlayer.empty())
    {
        internalFillPlayerStats(req, ggiResp, randomAthleteCount1, randomAthleteCount2, statsTable->mKickReturnPlayer);
    }
}
void FranchiseInstance::fillFieldPlayerStat(Franchise::SubmitFranchiseGameStatsRequest& req, Franchise::GetGameplayInformationResponse& ggiResp)
{
    ...
    StatsTables* statsTable = mOwner->getStatsTable();
    if (statsTable && !statsTable->mFieldPlayer.empty())
    {
        internalFillPlayerStats(req, ggiResp, randomAthleteCount1, randomAthleteCount2, statsTable->mFieldPlayer);
    }
}
...
BlazeRpcError FranchiseInstance::submitScoringSummary (Franchise::SubmitScoringSummaryRequest& req, Franchise::TeamId homeTeamId, Franchise::TeamId awayTeamId)
{
    ...
    {
        const ScoringSummary &summary = stats.mScoringSummary[rand() % stats.mScoringSummary.size()];
        ScoreDescription *score = new ScoreDescription;
        score->setOrder(i);
        score->setPeriod((Franchise::PeriodType)summary.stat0);
        score->setDescription(summary.stat1.c_str());
        score->setHomeScore((uint16_t)summary.stat2);
        score->setAwayScore((uint16_t)summary.stat3);
...
{
    StatsTables* statsTable = mOwner->getStatsTable();
    if (statsTable && !statsTable->mAthleteStatsByGame.empty())
    {
        const IntArray &stats = statsTable->mAthleteStatsByGame[rand() % statsTable->mAthleteStatsByGame.size()];
...
void FranchiseInstance::fillSubmitAthleteGameStatsRequest(Franchise::SubmitAthleteGameStatsRequest& req)
{
    ...
    int32_t randomNumEntries1 = rand() % 40 ;    // 40 is just a random number
    for (int32_t i=0; i < randomNumEntries1; i++)      
    {
        fillAthleteStatsByTeam(athleteStatsT1, (Franchise::AthleteId)i);
    }

    int32_t randomNumEntries2 = rand() % 40 ;    // 40 is just a random number
...
BlazeRpcError FranchiseInstance::submitTeamGameStats (Franchise::SubmitTeamGameStatsRequest& req)
{
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        eastl::vector<int> &row = stats.mTeamStatsByGame[rand() % stats.mTeamStatsByGame.size()];
...
{
    // NOTE: FOR EVERY RPC CALL, CHECK RPCRESULT ERROR CODE, PLUS ADD THE STANDARD RPC DELAY AND ERROR LOGGING
    // PLUS ADD IN THE RPC TIMING
            ...
            for (; iter != gameList->end(); iter++)
            {
                ...
                if (state == 1) // unplayed
                {
                    ...
                    if (mOwner->getStatsEnabled())
                    {
                        ...
                        sfsReq.setState(FRANCHISE_STATE_REGULAR_SEASON);

                        statsTable = mOwner->getStatsTable();
                        if (statsTable && !statsTable->mTeamStatsByGame.empty())
                        {
    ...
    else
    {
        BLAZE_INFO (BlazeRpcLog::franchise, "[%d] Cannot advance week as franchise=%d is in state=%d, weekid=%d, seasonid=%d",
            mId, req.getFranchiseId(), curState, req.getWindowId(), req.getSeasonId());
    }
}
...
{
    //FranchiseState curState = FRANCHISE_STATE_REGULAR_SEASON;
    FranchiseState nextState = FRANCHISE_STATE_NONE;

    // If you need more complex logic, add this back in.
    //if (mCurrentFranData && franId == mCurrentFranData->fid)
    //{
    //    curState = mCurrentFranData->state;
    //}
    //else // loop thru all frandata structs and grab state - todo
    //{
    //}
...
{
    FranchiseIdDataVectorType::iterator iterFrans = mVecFranIdData.begin();
    eastl::vector<FranchiseIdData*> vecFranData;
    ...
    for (; iterFrans != mVecFranIdData.end(); iterFrans++)
    {
        ...
        for (; iterMembers != franData->memberData.end(); iterMembers++)
        {
            ...
            {
                vecFranData.push_back (franData);
            }
        }
    }

    eastl::vector<FranchiseIdData*>::size_type size = vecFranData.size();
    ...
    if (size > 1)
    {
        ...
        {
            mCurrentFranData = vecFranData[indexIntoVector];
            return mCurrentFranData->fid;
            }
        }
    else if (size == 1)
    {
        mCurrentFranData = vecFranData[0];
        return mCurrentFranData->fid;
    }
...
{
    FranchiseIdDataVectorType::iterator iter = mVecFranIdData.begin();
...
{
    eastl::vector<FranchiseId>::size_type size = mVecFranIdData.size();
    if (size > 0)
    {
        ...
        return indexIntoVector > 0 ? mVecFranIdData[indexIntoVector]->fid : 0; // sub 1 to get real 0-based index
    }
    return 0;
}
...
FranchiseId FranchiseInstance::getFranchiseWeAreNotAMemberOf ()
{
    ...
    if (size > 0)
    {
        while (!done && numAttempts++ < 50)
        {
            ...
            for (iterFrans = mVecFranIdData.begin(); iterFrans != mVecFranIdData.end() && !failed; iterFrans++)
            {
...
void FranchiseInstance::findMembersInOtherFranchises (FranchiseId franId, eastl::vector<MemberData*>& membersVec, bool fransWeCreatedOnly)
{
    ...
    for (; iterFrans != mVecFranIdData.end(); iterFrans++)
    {
        ...
        FranchiseIdData* franData = *iterFrans;
        if (franData && franData->fid != franId)
        {
...
FranchiseId FranchiseInstance::getFranchiseWithFantasyDraftSetting (bool fantasyDraftDesired, bool needGM, BlazeId bzidDesired)
{
    ...
    {
        FranchiseIdData* data = *iterFrans;
        if (data && data->fantasyDraftIsOn == fantasyDraftDesired)
        {
