...
- (void)startAnimation
{
    if (!animating && !self.displayLink)
    {
        CADisplayLink *link = nil;
        link = [CADisplayLink displayLinkWithTarget:self selector:@selector(drawFrame)];
        [link setFrameInterval:animationFrameInterval];
...
- (void)drawFrame
{
	if (g_bInBackground || self.eagl == nil)
        return;
