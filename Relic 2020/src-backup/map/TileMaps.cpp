...
int32_t TileMaps::GetTileIdAtWorld(ITileMapModel *model, const core::Point &at)
{
...
void TileMaps::SetTileIdAtWorld(ITileMapModel *model, const core::Point &at, int32_t id)
{
    core::Point p     = GetTileLocationAtWorld(model->GetTileSize(), at);
    core::Size  mapsz = model->GetMapSizeInTiles();
    if (p.x < 0 || p.y < 0 || p.x >= mapsz.width || p.y >= mapsz.height)
