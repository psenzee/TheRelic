...
static void RenderTile(RenderContext &context, ITileMapModel *model, const Vector3 &at, bool clip, int x, int y, bool renderDefaults, bool forceDefault)
{
    if (!renderDefaults && forceDefault)
        return;
    ...
    if (renderDefaults && (forceDefault || x < 0 || y < 0 || x >= sz.width || y >= sz.height))
    {
        ITile *tile = model->GetTileSet()->GetDefaultTile();
        if (tile)
            r = tile->GetGameObject();
    }
    if (r)
    {
...
void TileMapRenderer::RenderRange(RenderContext &context, ITileMapModel *model, const Vector3 &at, const core::Rectangle *area_, bool renderDefaults, const BitMap2d *mask)
{
    ...
    for (int j = area.y, jsz = area.y + area.height; j < jsz; j++)
        for (int i = area.x, isz = area.x + area.width; i < isz; i++)
             RenderTile(context, model, at, false, i, j, renderDefaults, mask && !mask->get(i, j));
}

core::Point GetTileLocationFrom(const core::Size &tilesz, const Vector3 &at, const Vector3 &position)
{
    return core::Point(int32_t((position.x - at.x) / tilesz.width),
                       int32_t((position.y - at.y) / tilesz.height));
}
...
void TileMapRenderer::Render(RenderContext &context, ITileMapModel *model, const Vector3 &at, const AABox &bounds, bool renderDefaults, const BitMap2d *mask)
{
    core::Rectangle range = FindRange(model, at, bounds, context.camera);
    RenderRange(context, model, at, &range, renderDefaults, mask);
}
