...
bool Bundle::ReadIndex()
{
    ...
    for (int i = 0; i < header.entryCount; i++)
    {
        ...
        memcpy(indexName, p + sizeof(entry), entry.entrySize - sizeof(uint32_t));
        //printf("name: %s\n", indexName);
        p += entry.entrySize + sizeof(uint32_t);
        mIndex[indexName] = (char *)mData.data + entry.fileOffset + sizeof(uint32_t) + sizeof(uint32_t);
    }
    return true;
}
