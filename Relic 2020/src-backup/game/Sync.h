...
public:
    ...
    Character         *GetPlayer() { return mPlayer; }
...
private:
    ...
    static bool OnReceiveCharacterUpdates(const char *message, int length, void *user);

    enum { MAX_CHARACTERS = 32768, MAX_PLAYERS = 4 };
    ...
    bool                              mPlayerUpdated[MAX_PLAYERS];

    // a more efficient implementation would be better
    // ideally a hash map
    std::map<int, CharacterSyncInfo>  mReceived;
