...
static int Ui_SendRisingMessage(lua_State *lua)
{
    ...
    GetGameUi().SendBallMessage(lua_tostring(lua, -2), static_cast<int>(lua_tointeger(lua, -1)));
    return 0;
}
...
static int Text_DrawString(lua_State *lua)
{
    ...
    GetGameUi().DrawString(GetGlobalRenderContext(), text, Vector2(x, y), Vector4(1.f, 1.f, 1.f, alpha), size);
    return 0;
}
...
static int Text_DrawStringCentered(lua_State *lua)
{
    ...
    float x    = static_cast<float>(lua_tonumber(lua, -4)), y     = static_cast<float>(lua_tonumber(lua, -3)),
          size = static_cast<float>(lua_tonumber(lua, -2)), alpha = static_cast<float>(lua_tonumber(lua, -1));
    GetGameUi().DrawString(GetGlobalRenderContext(), text, Vector2(x, y), Vector4(1.f, 1.f, 1.f, alpha), size, true, GlyphWriter::TEXT_JUSTIFY_CENTER);
    return 0;
}
...
static int Text_GetSize(lua_State *lua)
{
    ...
    Vector2 size(GetGameUi().GetSize(text, Vector2(scale, scale)));
...
static int Ui_DrawStringGlow(lua_State *lua)
{
    ...
    float x    = static_cast<float>(lua_tonumber(lua, -4)), y     = static_cast<float>(lua_tonumber(lua, -3)),
          size = static_cast<float>(lua_tonumber(lua, -2)), alpha = static_cast<float>(lua_tonumber(lua, -1));
    GetUiCore()->DrawStringGlow(text, Vector2(x, y), JUSTIFY_LEFT, alpha, size);
    return 0;
}
...
static int Ui_DrawStringCentered(lua_State *lua)
{
    ...
    float x    = static_cast<float>(lua_tonumber(lua, -4)), y     = static_cast<float>(lua_tonumber(lua, -3)),
          size = static_cast<float>(lua_tonumber(lua, -2)), alpha = static_cast<float>(lua_tonumber(lua, -1));
    GetUiCore()->DrawString(text, Vector2(x, y), JUSTIFY_CENTER, alpha, size);
    return 0;
}
...
static int Ui_DrawStringCenteredGlow(lua_State *lua)
{
    ...
    float x    = static_cast<float>(lua_tonumber(lua, -4)), y     = static_cast<float>(lua_tonumber(lua, -3)),
          size = static_cast<float>(lua_tonumber(lua, -2)), alpha = static_cast<float>(lua_tonumber(lua, -1));
    GetUiCore()->DrawStringGlow(text, Vector2(x, y), JUSTIFY_CENTER, alpha, size);
    return 0;
}
...
static int Ui_GetStringSize(lua_State *lua)
{
    ...
    const char *text  = lua_tostring(lua, -2);
    float scale = static_cast<float>(lua_tonumber(lua, -1));
    Vector2 size(GetUiCore()->GetStringSize(text, scale));
    lua_pushnumber(lua, size.x);
    lua_pushnumber(lua, size.y);
    return 2;
}
...
static int Ui_DrawQuad(lua_State *lua)
{
    ...
    GetUiCore()->DrawQuad(lua_tostring(lua, -12), color, v2(start), v2(end), t0, t1);
    return 0;
}

static int Ui_DrawScreenQuad(lua_State *lua)
{
    luaL_checktype(lua, -6, LUA_TSTRING); // texture name
    luaL_checktype(lua, -5, LUA_TNUMBER); // alpha
    ...
    Vector4        color   = Vector4(1.0, 1.0, 1.0, (float)lua_tonumber(lua, -5));
    Vector2        t0      = Vector2((float)lua_tonumber(lua, -4),  (float)lua_tonumber(lua, -3)),
                   t1      = Vector2((float)lua_tonumber(lua, -2),  (float)lua_tonumber(lua, -1));

    GetUiCore()->DrawScreenQuad(lua_tostring(lua, -6), color, t0, t1);
    return 0;
}
...
static int Ui_DrawRing(lua_State *lua)
{
    ...
    GetUiCore()->DrawRing(lua_tostring(lua, -8), color,
...
static int Ui_DrawQuadRotate(lua_State *lua)
{
    ...
    GetUiCore()->DrawQuad(lua_tostring(lua, -13), color, v2(start), v2(end), t0, t1, angle);
...
static int Ui_DrawPanorama(lua_State *lua)
{
    ...
    GetUiCore()->DrawPanorama(lua_tostring(lua, -9), color, v2(start), v2(end), angle);

    return 0;
}

static int Ui_DrawScreenPanorama(lua_State *lua)
{
    luaL_checktype(lua, -4,  LUA_TSTRING); // texture name
    luaL_checktype(lua, -3,  LUA_TNUMBER); // alpha
    luaL_checktype(lua, -2,  LUA_TNUMBER); // offset
    luaL_checktype(lua, -1,  LUA_TNUMBER); // angle

    Vector4        color   = Vector4(1.0, 1.0, 1.0, (float)lua_tonumber(lua, -3));
    float          offset  = (float)lua_tonumber(lua, -2);
    float          angle   = (float)lua_tonumber(lua, -1);

    GetUiCore()->DrawScreenPanorama(lua_tostring(lua, -4), color, offset, angle);
...
static int Ui_Draw9Slice(lua_State *lua)
{
    ...
    GetUiCore()->Draw9SliceQuad(lua_tostring(lua, -10), color, v2(start), v2(end), cornerSize, middleRange);
    return 0;
}
...
static int Ui_SetRootControl(lua_State *lua)
{
    ...
    GetUiCore()->SetRootControl(control);
    return 0;
}

static int Ui_GetRootControl(lua_State *lua)
{
    IUiControl *control = GetUiCore()->GetRootControl();
...
{  
    ...
    IUiControl *control = GetUiCore()->CreateText(
...
{  
    ...
    bounds.minimum = Vector2((float)lua_tonumber(lua, -7), (float)lua_tonumber(lua, -6));
    bounds.maximum = Vector2((float)lua_tonumber(lua, -5), (float)lua_tonumber(lua, -4));
    IUiControl *control = GetUiCore()->CreateDialogText(
                              lua_tostring(lua, -9), lua_tostring(lua, -8), bounds,
                              (float)lua_tonumber(lua, -3), (float)lua_tonumber(lua, -2), 
                              lua_toboolean(lua, -1) != 0);
    lua_pushlightuserdata(lua, control);
...
{  
    ...
    IUiControl *control = GetUiCore()->CreateButton(
...
{  
    ...
    IUiControl *control = GetUiCore()->CreateImageButton(
...
{  
    ...
    IUiControl *control = GetUiCore()->CreateRenderableButton(
...
{  
    ...
    bounds.minimum = Vector2((float)lua_tonumber(lua, -6), (float)lua_tonumber(lua, -5));
    bounds.maximum = Vector2((float)lua_tonumber(lua, -4), (float)lua_tonumber(lua, -3));
    IUiControl *control = GetUiCore()->CreateMenu(
...
static int Ui_ScreenToDrawPosition(lua_State *lua)
{
    luaL_checktype(lua, -2,  LUA_TNUMBER); // at.x
    luaL_checktype(lua, -1,  LUA_TNUMBER); // at.y
    Vector2 pos = GetUiCore()->ScreenToDrawPosition(Vector2((float)lua_tonumber(lua, -2), (float)lua_tonumber(lua, -1)));
...
static int Ui_DrawToScreenPosition(lua_State *lua)
{
    luaL_checktype(lua, -2,  LUA_TNUMBER); // at.x
    luaL_checktype(lua, -1,  LUA_TNUMBER); // at.y
    Vector2 pos = GetUiCore()->DrawToScreenPosition(Vector2((float)lua_tonumber(lua, -2), (float)lua_tonumber(lua, -1)));
    lua_pushnumber(lua, pos.x);
    lua_pushnumber(lua, pos.y);
    return 2;
}
...
void RegisterLuaUiFunctions(lua_State *lua)
{
    ...
    lua_register(lua, "Ui_DrawScreenQuad",             Ui_DrawScreenQuad);
    ...
    lua_register(lua, "Ui_DrawScreenPanorama",         Ui_DrawScreenPanorama);
