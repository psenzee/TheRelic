...
protected:
    ...
    inline void            Release()     { core_assert(mReference >= 0); if (--mReference == 0) delete this; }
...
public:
    ...
    inline void            Push()                                             const { core_assert(mTable); if (mTable) mTable->Push(); }
    inline int             Size()                                             const { core_assert(mTable); return mTable ? mTable->Size() : 0; }

    inline void            AddFromStack()                                           { core_assert(mTable); if (mTable) mTable->AddFromStack(); }

    inline int             GetId()                                            const { core_assert(mTable); return mTable ? mTable->GetId() : -1; }
    inline lua_State      *GetState()                                         const { core_assert(mTable); return mTable ? mTable->GetState() : 0; }
