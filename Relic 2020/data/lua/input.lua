...
THUMBSTICK_FIELD_VECTOR_WEIGHT = 0.0 -- 0.5
...
function SetThumbStickMovement(index, x, y, timescale)
  ...
  if THUMBSTICK_IN_PROGRESS[index] or v3Distance(x, y, 0, tsx, tsy, 0) < tsr then
    local indir = { -(x - tsx) / tsr, -(y - tsy) / tsr, 0 }
...
-- print("TouchBegan: ", x, ", ", y)
  if not DISABLE_UI_INPUT then
    if not IsHudOn() or x > hudDisableEventLine then -- $HACK if hud is enabled, then only send events from the right half of the screen, to make the shield button work properly
