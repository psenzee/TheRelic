...
if value.s_seed == nil then value.s_seed = MakeRandomSeed() end
  local c = CreateNecromancer(value.s_type, value.s_hp, value.s_speed, value.s_x, value.s_y, value.s_seed)
--c:SetId(value.s_id)  
...
function Necromancer(c)
   ...
   -- get one hit on necromancer, then turn it off
   local data = c:data()
   if data.immolate ~= nil then
     data.immolate = nil
     data.immolator = nil
   end
   LaunchProjectile(c, "Fire", 50.0, 4)
end
...
function CreateNecromancer_Character(c, type, hp, speed, x, y, seed)
...
function CreateNecromancer(type, hp, speed, x, y, seed)
   local c = NewCharacter(-1, "Necromancer", type, x, y, 0.0)
   return CreateNecromancer_Character(c, type, hp, speed, x, y, seed)
end
