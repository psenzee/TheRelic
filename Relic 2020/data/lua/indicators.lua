...
function HasPlayerIndicatorName(id)
  local indicatorName = "Player" .. id .. "Indicator"
  local instance = GetIndicatorControl(indicatorName)
  if instance == nil then
    return false
  end
  local name = UiControl_GetText(instance)
print("has player indicator name:[" .. name .. "]")  
  return not string.isblank(name)
end
...
function InitializeIndicators()
  ...
  CreateIndicator("KillsIndicator")
  ...
  CreateIndicator("ShieldButton")
  CreateIndicator("TurnIndicator")
  CreateIndicator("HealPotionIndicator")
  CreateIndicator("HitMultiplierIndicator")
  CreateIndicator("HitCountIndicator")
  CreateIndicator("SpellTypeIndicator")  
  ShowIndicators()
end
...
function GetHealthPotionIndicatorCoors()
  local coors = BUTTON_HEALTH
  local offset = { 50, 20 }
  return { coors[1] + offset[1], coors[2] + offset[2], coors[3] + offset[1], coors[4] + offset[2] }
end

function GetShieldButtonIndicatorCoors()
  local coors = BUTTON_SHIELD
  -- local offset = { -240, 160 } -- adjust to UI coordinates
  local offset = { 10, 15, 10, -15  }
  return { coors[1] + offset[1], coors[2] + offset[2], coors[3] + offset[3], coors[4] + offset[4] }
end
...
function CreateIndicator(name)
  ...
  local iPadOffset = 0
  if GetPlatformIsiPad() then iPadOffset = 30; end
  -- local keyX = -20
  -- local keyRY =
  ...
  elseif name == "KillsIndicator"    then instance = Ui_CreateImageButtonJustified     (name, "barb-icon64", "", 0, 0,  45 + offset, 25,  70 + offset, 0.5, 4.0 * HUD_SCALE * HUD_INDICATOR_TEXT_SCALE, 0, 1, 1, 0, 0)
  ...
  elseif name == "RedKeyIndicator" then instance = Ui_CreateImageButtonJustified     (name, "keyButton128R", "", 0, -30,  45 + offset, -5,  70 + offset, 1.0, 4.0 * HUD_SCALE * HUD_INDICATOR_TEXT_SCALE, 0, 1, 1, 0, 0)
  elseif name == "BlueKeyIndicator" then instance = Ui_CreateImageButtonJustified     (name, "keyButton128B", "", 0, -30,  75 + offset, -5,  100 + offset, 1.0, 4.0 * HUD_SCALE * HUD_INDICATOR_TEXT_SCALE, 0, 1, 1, 0, 0)
  elseif name == "WhiteKeyIndicator" then instance = Ui_CreateImageButtonJustified     (name, "keyButton128W", "", 0, -30,  105 + offset, -5,  130 + offset, 1.0, 4.0 * HUD_SCALE * HUD_INDICATOR_TEXT_SCALE, 0, 1, 1, 0, 0)
  ...
  elseif name == "HealPotionIndicator" then
      coors = GetHealthPotionIndicatorCoors()
      instance = Ui_CreateRenderableButtonJustified(name, "Empty",       "", 2,
         coors[1], coors[2], coors[3], coors[4], 1.0, 8.0 * HUD_SCALE * HUD_BUTTON_TEXT_SCALE, 1.0 * HUD_SCALE * HUD_BUTTON_SCALE, 3.6 * HUD_BUTTON_ZSCALE)
--elseif name == "HitMultiplierIndicator" then instance = Ui_CreateRenderableButtonJustified(name, "Empty",       "", 2, 450, 260, 450, 260, 1.0, 9.0 * HUD_SCALE * HUD_BUTTON_TEXT_SCALE, 1.0 * HUD_SCALE * HUD_BUTTON_SCALE, 3.6 * HUD_BUTTON_ZSCALE); dynamicUpdate = false;
  elseif name == "HitMultiplierIndicator" then instance = Ui_CreateRenderableButtonJustified(name, "Empty",       "", 2, 260, 20 + HUD_TOP_OFFSET, 260, 20 + HUD_TOP_OFFSET, 1.0, 9.0 * HUD_SCALE * HUD_BUTTON_TEXT_SCALE * HUD_SCALE_FACTOR, 1.0 * HUD_SCALE * HUD_BUTTON_SCALE, 3.6 * HUD_BUTTON_ZSCALE); dynamicUpdate = false;
  elseif name == "HitCountIndicator" then instance = Ui_CreateRenderableButtonJustified(name, "Empty",       "", 2, 420, 300, 420, 300, 1.0, 7.0 * HUD_SCALE * HUD_BUTTON_TEXT_SCALE, 1.0 * HUD_SCALE * HUD_BUTTON_SCALE, 3.6 * HUD_BUTTON_ZSCALE)  
  elseif name == "SpellTypeIndicator" then 
-- instance = Ui_CreateRenderableButtonJustified(name, "Empty",       "", 0, 420, 200, 420, 200, 1.0, 5.0 * HUD_SCALE * HUD_BUTTON_TEXT_SCALE, 1.0 * HUD_SCALE * HUD_BUTTON_SCALE, 3.6 * HUD_BUTTON_ZSCALE)
-- instance = Ui_CreateButton(name, "border88_dark", "+Portal Near Forge", 20, 205, 220, 225, 1.0, 18.75)
   instance = Ui_CreateRenderableButtonJustified(name, "Empty", "", 0, 40, 25 - iPadOffset, 90, 25 - iPadOffset, 1.0, 6.0 * HUD_SCALE * HUD_BUTTON_TEXT_SCALE, 1.0 * HUD_SCALE * HUD_BUTTON_SCALE, 3.6 * HUD_BUTTON_ZSCALE)
  ...
  elseif name == "SaveButton"        then
    ...
    instance = Ui_CreateRenderableButton(name, "Save", "", -100, -100, -100, -100, 0.5, 8.0 * HUD_SCALE * HUD_BUTTON_TEXT_SCALE, 0.10 * HUD_SCALE * HUD_BUTTON_SCALE, 3.6 * HUD_BUTTON_ZSCALE)
    RegisterUiListener(name, "SaveHudListener", "")
  elseif name == "ShieldButton"        then
    coors = GetShieldButtonIndicatorCoors()
    instance = Ui_CreateImageButton(name, "shield1", "", coors[1], coors[2], coors[3], coors[4], 1.0, 18.75, 0, 1, 1, 0, 0)
    RegisterUiListener(name, "ShieldListener", "")
  end
...
function ShowIndicatorForce(name)
  if INDICATORS[name] == nil then
    CreateIndicator(name)
  end
  local instance = INDICATORS[name].instance
  _SetVisibleForce(instance, INDICATORS[name].visibilityOverride)
end

function SetIndicatorCountMinNoText(name, prefix, suffix, count, minimum)
  -- SetIndicatorText(name, prefix, suffix, tostring(count))
  if count == nil or count < minimum then
    HideIndicator(name)
  else
    ShowIndicator(name)
  end
...
function SetIndicatorCountMin(name, prefix, suffix, count, minimum)
  SetIndicatorText(name, prefix, suffix, tostring(count))
  if count == nil or count < minimum then
    HideIndicator(name)
  else
...
function SetIndicatorCount(name, prefix, suffix, count)
  SetIndicatorCountMin(name, prefix, suffix, count, 1)
end
...
function SetIndicatorText(name, prefix, suffix, text)
  ...
  if indicator.text ~= textstr then
    UiControl_SetText(indicator.instance, textstr)
    ShowTip(name)    
    --indicator.text = text
    indicator.text = textstr
