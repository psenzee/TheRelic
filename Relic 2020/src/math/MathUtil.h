...
public:
    ...
    inline Ray() {}
    inline Ray(const Tuple3f &origin, const Tuple3f &direction) : origin(origin), direction(direction) {}
    inline Ray(const Ray &other) : origin(other.origin), direction(other.direction) {}
    
    inline Tuple3f at(float t) const { return origin + direction * t; }
};
