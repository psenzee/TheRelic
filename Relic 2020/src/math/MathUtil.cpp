...
float MathUtil::NormalizeAngle2Pi(float x)
{
    if (x >= math::TWOPIf) return fmodf(x, math::TWOPIf);
    const float TWOPI_INV = 1.0f / math::TWOPIf;
    return x >= 0.f ? x : x + (floorf(-x * TWOPI_INV) + 1.f) * math::TWOPIf;
}

float MathUtil::NormalizeAnglePiNegPi(float x)
{
    if (x < math::PIf && x > -math::PIf)
        return x;
    x = NormalizeAngle2Pi(x);
    return (x < math::PIf) ? x : x - math::TWOPIf;
}    

bool MathUtil::AreAnglesClose(float a, float b, float tolerance)
{
    return fabsf(NormalizeAngle2Pi(a)     - NormalizeAngle2Pi(b))     <= tolerance ||
           fabsf(NormalizeAnglePiNegPi(a) - NormalizeAnglePiNegPi(b)) <= tolerance;
}

float MathUtil::GetAngleDifference(float a, float b)
{
    float d2Pi     = NormalizeAngle2Pi(a)     - NormalizeAngle2Pi(b),
          dPiNegPi = NormalizeAnglePiNegPi(a) - NormalizeAnglePiNegPi(b);
    return fabsf(dPiNegPi) < fabsf(d2Pi) ? dPiNegPi : d2Pi;
}
...
float MathUtil::GetAngle(const Vector3 &a, const Vector3 &b, float defaultAngle)
{
    float alensq = a.lengthsq(), blensq = b.lengthsq();
    if (alensq == 0.0f || blensq == 0.0f)
        return defaultAngle;
    Vector3 va(a), vb(b);
    if (!math::eq(alensq, 1.0f))
        va.normalize();
    if (!math::eq(blensq, 1.0f))
        vb.normalize();
    float angle = acos(va.dot(vb));
    // we need the sign of the cross product here to tell us which direction to rotate
    float c = vb.cross(va).z;
    if (c < 0.f)
        angle = -angle;
    if (c == 0.f)
        angle = 0.0f;
