...
static const char *GetRealAnimationName(const char *key, bool walkAndAttack)
{   
    struct StringPair
    {
        const char *key;
    ...
    StringPair animationsWalkAttack[] =
    {
      { "Run",                "Run" },
      { "RunTurn",            "Run" },
      { "RunAttack",          "WalkAttack" },
    ...
    StringPair animationsNoWalkAttack[] =
    {
      { "Run",                "Run" },
      { "RunTurn",            "Run" },
      { "RunAttack",          "Attack" },
    ...
    StringPair *animations = walkAndAttack ? animationsWalkAttack : animationsNoWalkAttack;
...
AnimationBehavior::AnimationBehavior() 
  : AbstractBehavior(TYPE), mAngle(0.f), mTurn(false), mTurnAngle(0.f),
    mTurnForce(0.f), mCanMove(true), mIsAlive(true), mIsAttackSet(false), mWalkAndAttack(false), 
    mAttackTypeCount(1), mDirection(0.f, 1.f, 0.f), mInitialized(false), mAccumulatedForce(0.f),
    mAccumulatedForceChange(0.f, 0.25f), mDisableMoveCount(0), mAttackCount(0), mActionReset(false),//mCenter(0.05f),
...
void AnimationBehavior::SetAction(const char *value, bool reset)
{
    ...
    if (value && strcmp(value, "Attack") == 0)
    {
//        mAttackCount++;
    }
    mActionReset = true;
