...
void MeleeAttack::Update(const GameTime &time)
{
    ...
    if (IsAbleToAttack())
    {
        ...
        {
           // bool inRange = GetAttackDistance() < (self->GetRadius() + to->GetRadius()) * 1.5f;
            bool inRange = GetAttackDistance() < (self->GetRadius() + to->GetRadius()) * 2.0f;			
            ...
            {
                if (!IsAutoAttack() || (self->IsFacing(GetAttackTarget()->GetPosition(), 1.552f) && inRange)) // allow pointless attacks if not auto attack
                {
