...
void Game::Autosave()
{
    LuaCall(mGameThread->GetLuaState(), "Autosave", 0, 0);
}
...
void Game::Visit(const core::Rectangle &range)
{
    if (mLevels)
    {
        Level *level = mLevels->GetLevel();
        if (level)
            level->Visit(range, *this);
    }
}
...
bool Game::Update(const GameTime &time)
{
    ...
    GraphicsDevice *device = GraphicsDevice::GetInstance();
    //printf("\nGame::Update count %d [running? %s]\n", gameUpdateCount, mGameInProgress ? "yes" : "no");
    if (mGameInProgress)
    {
        gameUpdateCount++;
// Multiplayer currently disabled
//      if (mFrames % 2 == 0) // only do this every other frame
//          mMultiplayer->Update();        
  ...
  //    GetGlobalUiEventDispatcher()->Update(); // in GameState::Update()
