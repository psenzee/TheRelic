...
int PackRpc(lua_State *lua, char *buffer)
{
    ...
    p += RPC_PREFIX_SIZE;
    int from = /*lua_tointeger(lua, -4)*/GetThisUserId(), to = static_cast<int>(lua_tointeger(lua, -3));
    PackChar(&p, from); // thisUserId
    PackChar(&p, to); // toUserId
...
int UnpackLuaValueString(const char **from, String &str, const Dictionary *dictionary)
{
...
static inline bool SkipSerializePair(const LuaValue &key)
{
    // don't serialize key/value pairs with keys that start with '_' - that's how we mark members as transient
    return key.GetType() == LuaValue::LUATYPE_STRING &&
           !key.GetString().empty() && *key.GetString().c_str() == '_';
}
...
//printf("Resolved function name '%s' for packing\n", fn.GetFunction());
...
static LuaTable UnpackLuaList(const char **from, lua_State *lua, const Dictionary *dictionary)
{
    ...
    {
        core_assert(false && "bad serialized lua table (list)!");
...
static LuaTable UnpackLuaMap(const char **from, lua_State *lua, const Dictionary *dictionary)
{
    ...
    {
        core_assert(false && "bad serialized lua table (map)!");
        return LuaTable::Null();    
    }
    LuaTable table = LuaTable::New(lua);
    while (**from != '\xff')
    ...
    case ABSTRACT_PACK_NUMBER: 
        {
            ...
            core_assert(false && "bad serialized lua number!");
            break;
        }
    case ABSTRACT_PACK_STRING:
        {
            ...
            if (UnpackLuaValueString(from, s, dictionary) >= 0)
                return LuaValue::_String(s);
            core_assert(false && "bad serialized lua string!");
            break;
        }
    case ABSTRACT_PACK_FUNCTION:
        {
            ...
            core_assert(false && "bad serialized lua function (string)!");
            break;
        }
    case ABSTRACT_PACK_TABLE:
        {
            ...
            core_assert(false && "bad serialized lua table!");
            break;        
        }
    default: break;
    }
    core_assert(false && "bad serialized lua value!");
    return LuaValue();
}
