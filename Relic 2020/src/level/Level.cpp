...
void GetNamed(std::vector<Map *> &map, const std::vector<MapDescription> &descriptions, const char *name, std::vector<core::Point> &items)
{
    ...
    for (int32_t j = 0; j < mapSizeInTiles.height; j++)
    {
        for (int32_t i = 0; i < mapSizeInTiles.width; i++)
        {
            core::Point p(i, j);
...
Character *Level::GetCharacter(int32_t i)
{ 
...
size_t Level::GetMovableCount() const
{
    return !mDynamic ? 0 : mDynamic->Count();
}
...
{
    return core::Point(int32_t(player.x - map.x), int32_t(player.y - map.y));
}
...
int32_t Level::GetTileIdAt(Map *map, const Vector3 &position)
{
...
static int32_t GetTileEventCountAt(std::vector<Map *> &maps, const std::vector<MapDescription> &descriptions, const Vector3 &position)
{
    int32_t count = 0;
    std::vector<MapDescription>::const_iterator di = descriptions.begin();
...
static const char *GetTileEventAt(std::vector<Map *> &maps, const std::vector<MapDescription> &descriptions, const Vector3 &position, int32_t index)
{
    int32_t count = 0;
    std::vector<MapDescription>::const_iterator di = descriptions.begin();
    for (std::vector<Map *>::iterator i = maps.begin(), e = maps.end(); i != e; ++i)
...
int32_t Level::GetTileEventCountAt(const Vector3 &position)
{
    return ::GetTileEventCountAt(mMaps, mOptions->maps, position);
}

const char *Level::GetTileEventAt(const Vector3 &position, int32_t index)
{
...
static float GetCollisionWeight(const Vector2 &at, void *user)
{
    ...
    Vector3  resolve;
    int32_t id = map->GetTileIdAt(at);
    float weight = COLLIDE_WEIGHT;	
    if (id == 6) return COLLIDE_WEIGHT; // an optimization here..
    if (id >= 64 && id <= 91) // doorways, make them easy to pass through
        weight *= 0.25f;
    ICollidable::Classification classification = map->Collision(v3(at, 10.f), 32.f, resolve);    
    switch (classification)
    {
    case ICollidable::CLASS_IN:  return weight;
    case ICollidable::CLASS_ON:  return weight * 0.5f;
...
{
    int value = GetLevelNumber() * 991 + (int(player.x) / 1024) * 3 + (int(player.y) / 1024) * 5;
    core::Random r(value);
    ...
    if (mMaps[LABEL_MAP])
    {
        int32_t value = mMaps[LABEL_MAP]->GetTileIdAt(v2(player));
        switch (value)
        {
        case 32896: return "Pandora's Chasm";
      ...
      if (last != value)
{
  ...
  last = value;
     ...
     */
    return mPlaceNameGenerator.GetPlaceNameAt(32 /*$TODO TEMPORARY */, static_cast<int32_t>(fabsf(player.x / 1024.f)),
                                                                       static_cast<int32_t>(fabsf(player.y / 1024.f)));
}

static int32_t _tmpGetDownStairsFloorType(int32_t floorType)
{
    switch (floorType)
    {
...
static void _tmpResolveDownStairs(std::vector<Map *> &maps)
{
    ...
    for (int32_t j = 0; j < sz.height; j++)
        for (int32_t i = 0; i < sz.width; i++)
...
Level::Level(ContentLoader *loader, const char *path, int32_t level, Multiplayer *multiplayer) : mDynamic(0), mLevelNumber(level), mMultiplayer(multiplayer), mSync(0), mVectorField(0), mVisited(0), mVisitedMap(0)
{
    ...
    VectorFieldMap field(0.f, COLLIDE_WEIGHT, 7/*4*/, 3/*5*/);
    field.SetDimensions(Vector2(0.f, 0.f) /* we always start at 0, 0 in this game */,
                                mapDimensions,
                                static_cast<int32_t>(tileSize.width / 16.f));
    int32_t time = GetCurrentTimeMs();
...
Vector3 Level::GetSpawnablePoint(int32_t index)
{
...
int32_t Level::GetNamedPointCount(const char *name)
{
    const std::vector<core::Point> &items = GetItems(name);
    return static_cast<int32_t>(items.size());
}

Vector3 Level::GetNamedPoint(const char *name, int32_t index)
{
    const std::vector<core::Point> &items = GetItems(name);
    if (items.empty())
        return Vector3();
...
char *Level::GetVisitedData(int32_t *size)
{
...
bool Level::SetVisitedData(const char *data, int32_t size)
{
    int32_t size1    = 0,
            size2    = 0;
...
static void SetPerimeter(BitMap2d *map, const core::Rectangle &range, int32_t margin, bool value)
{
    for (int32_t i = -margin; i < range.height + margin * 2; i++)
    {
        map->set(range.x - margin,                   i + range.y, value);
        map->set(range.x + range.width + margin - 1, i + range.y, value);
    }
    for (int32_t i = -margin + 1; i < range.width + margin * 2 - 1; i++)
    {
...
void Level::Visit(const core::Rectangle &range, IEventListener &listener)
{
    enum { CONSTRAINED_COUNT_ENEMY_TYPE = 1111, MAX_ENEMY_COUNT = 25 };

    int32_t offset = 0;
    for (int32_t j = -offset; j < range.height + offset * 2; j++)
    {
        for (int32_t i = -offset; i < range.width + offset * 2; i++)
        {
    ...
    int32_t margin = 4, outermargin = margin + 4;
    int32_t enemyCount = mDynamic->GetCountOfType(CONSTRAINED_COUNT_ENEMY_TYPE,
...
core::Rectangle Level::GetRenderRange() const
{
    return mMaps[BASE_MAP]->GetRenderRange();
}
...
int32_t Level::GetFreeCharacterId() const
{ 
    enum { LOWEST_RESERVED_CHARID = 32 };
    int32_t id = mDynamic->GetNextFreeId(-1), playerId = mSync->GetPlayerId();
    if (playerId < 0)
        playerId = 0;
    while ((id != -1 && id < LOWEST_RESERVED_CHARID) || (id != -1 && (id % Multiplayer::MAX_PLAYERS != playerId)))
    {
...
int32_t Level::GetCharacterIdOwner(int32_t id)
{
...
void Level::Draw(RenderContext &context, const GameTime &time, const PauseState &pause)
{
    if (!pause.render)
    {
        for (int32_t i = 0; i < mOptions->maps.size(); i++) {
        ...
        if (!pause.characters && mDebrisMap) {
    ...
    for (std::vector<Map *>::iterator i = mMaps.begin(), e = mMaps.end(); i != e; ++i)
    {
        ...
        if (tile && (*di).events)
        {
            ...
            if (r)
            {
                ...
                int32_t     count  = elist.GetEventCount();
                if (count)
                {
