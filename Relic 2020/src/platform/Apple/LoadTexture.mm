...
{
    const char *path = globalTranslatePath(filename);
...
static uint32_t LoadPVRTexture(const char *filename, core::Size *size)
{
  	uint32_t _name = 0;
...
uint32_t LoadPNGTexture(const char *filename, core::Size *size)
{
    ...
    printf("PNG name %s\n", name);
    UIImage *uiImage = [[UIImage alloc] initWithContentsOfFile:[NSString stringWithUTF8String:globalTranslatePath(name)]];
...
//      printf("Loaded texture '%s' with dimensions %ux%u\n", name, width, height);
    ...
    //[uiImage release]; // HERE for testing
    //return 0;
...
static uint32_t _LoadTexture(const char *filename, core::Size *size)
{
    ...
    case TYPE_PVR: textureId = LoadPVRTexture(filename, size); break;
 	case TYPE_UNKNOWN: printf(" (unknown texture type) "); break;
    case TYPE_NOT_FOUND: printf("."); break;
    }
    return textureId;
}

uint32_t AttemptToLoadInSequence(const char *filename, core::Size *size, const char **types)
{
	uint32_t textureId = 0;
...
uint32_t LoadTexture(const char *filename, core::Size *size)
{
