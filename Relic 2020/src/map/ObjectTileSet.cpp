...
void ObjectTileSetLoader::RemoveAllTileSets()
{
	/*
	std::vector<std::string> names;
	for (std::map<std::string, ITileSet *>::const_iterator i = cache.begin(), e = cache.end(); i != e; ++i)
		names.push_back((*i).first);
	for (std::vector<std::string>::const_iterator i = names.begin(), e = names.end(); ++i)
		RemoveTileSet((*i).c_str());
    */
	for (std::map<std::string, ITileSet *>::const_iterator i = cache.begin(), e = cache.end(); i != e; ++i)
	{
		ITileSet *ts = (*i).second;
		if (ts) delete ts;
	}
	cache.clear();
}
