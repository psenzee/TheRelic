...
// GetRandomSpot
inline static float Interpolate(float t, float lo, float hi)
{
    float d = hi - lo;
    return t * d + lo;
}

inline static Vector3 GetRandomVector3(core::Random &r, const Vector3 &lo, const Vector3 &hi)
{
    return Vector3(Interpolate(r.realf(), lo.x, hi.x),
                   Interpolate(r.realf(), lo.y, hi.y),
                   Interpolate(r.realf(), lo.z, hi.z));
}
/*
Spawnable on any tile with open space, just respect colliders (can use vector map to find open spaces)
  Have a bool GetRandomSpotAt(const core::Point &tile, Vector3 &at) call
  - then into lua
  - return true/false if unable to get one (ie, no open space)
*/

bool Map::GetRandomSpot(core::Random &r, float radius, const Vector3 &lo, const Vector3 &hi, Vector3 &at)
{
    // extern bool ::MapCollider(const Vector3 &p, float radius, Vector3 &resolve, void *user); // from Map.cpp; user is an instance of Map
    // extern Vector3 GetRandomVector3(core::Random &r, const Vector3 &lo, const Vector3 &hi); // right now from audio code, needs to be somewhere generic, like MathUtil
    enum { TRIES = 16 };
    for (int i = 0; i < TRIES; i++)
    {
        Vector3 p(GetRandomVector3(r, lo, hi)), unused;
        if (!MapCollider(p, radius, unused, this))
        {
            at = p;
            return true;
        }    
...
Map::Map(ITileMapModel *model) : MapModel(model), Start(0, 0)
{        
...
void Map::Update(RenderContext &context, const GameTime &time)
{
    ...
    MapModel->Update(time, mRenderRange);
}
...
void Map::Draw(RenderContext &context, const GameTime &time, bool renderDefaults, const BitMap2d *mask)
{
    core::Size framesz = context.device.GetFrameSize(),
               tilesz  = MapModel->GetTileSize();
    TileMapRenderer::RenderRange(context, MapModel, Position, &mRenderRange, renderDefaults, mask);
}
...
Vector2 Map::GetWorldLocationAtTile(const core::Point &p) const
{
    core::Point at = TileMaps::GetWorldLocationAtTile(MapModel->GetTileSize(), p);
    return Vector2(float(at.x) + Position.x, float(at.y) + Position.y);
}
