...
const char *VertexField::_attributeName(const char *type, unsigned index)
{
    constexpr size_t LENGTH = 31;
    static char buffer[LENGTH + 1] = { 0 };
    if (index) { snprintf(buffer, LENGTH, "a_%s_%u", type, index); }
    else       { snprintf(buffer, LENGTH, "a_%s",    type); }
    return buffer;
}

int VertexField::locationIdForName(const char *name)
{
    static constexpr const char *types[] = { "position", "normal", "texture", "color" };
    if (name[0] == 'a' || name[1] == '_') {
        for (size_t i = 0; i < size_t(MAX_ITEMS_PER_FIELD) * size_t(_TYPE_MAX); i++) {
            const char *attrib = _attributeName(types[i % size_t(_TYPE_MAX)], unsigned(i) / _TYPE_MAX);
...
const char *VertexField::nameForLocationId(unsigned locationId)
{
    static constexpr const char *types[] = { "position", "normal", "texture", "color" };
    size_t modulo = (sizeof(types) / sizeof(types[0]));
    return _attributeName(types[locationId % modulo], locationId / modulo);
}

int VertexField::locationIdForTypeAndIndex(Type type, unsigned index)
{
    return locationIdForName(attribute(type, index));
}
...
const char *VertexField::attribute(Type type, size_t index)
{
    ...
    if (!*buffer) { strcpy(buffer, _attributeName(name(type), index)); }
    return buffer;
}
...
const char *VertexFormat::_error_messages(Error e)
{
    ...
    static const char *text[] = {
        ...
        "parse error: bad triangle type, expected L for list, S for strip, or F for fan",
