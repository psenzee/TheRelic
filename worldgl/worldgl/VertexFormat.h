...
struct VertexField
{
    ...
    static                  int         locationIdForName(const char *name);
    static                  const char *nameForLocationId(unsigned locationId);
    static                  int         locationIdForTypeAndIndex(Type type, unsigned index);

private:

    static                  const char *_attributeName(const char *type, unsigned index);
...
struct VertexFormat
{
    ...
    enum Type       { TRI_NONE = 0, TRI_LIST, TRI_STRIP, TRI_FAN };
