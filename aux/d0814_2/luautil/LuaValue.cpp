...
bool LuaValue::Verify(Type type) const
{
    if (mType != type)
    {
        core_assert(mType == type && "types are not equal!");
    }
    return mType == type;
}
...
void LuaValue::SetString(const ::String &value)
{
    SetNull();
    mString = value;
...
const String &LuaValue::GetString() const
{
...
void LuaValue::SetLightUserData(void *value)
{
    SetNull();
    mLightUserData = value;
    mType          = LUATYPE_LIGHTUSERDATA;
}
...
void LuaValue::SetUserData(int value)
{
    SetNull();
    mUserData = value;
    mType     = LUATYPE_USERDATA;
}
...
int64_t LuaValue::GetInt64() const
{
    ...
    {
        core_assert(false && "value is not an int64!");
    }
    return value;
}
...
uint64_t LuaValue::GetUInt64() const
{
    ...
    {
        core_assert(false && "value is not a uint64!");
    }
    return value;
}

String LuaValue::ToString(lua_State *lua) const
{
    Push(lua);
    size_t size = 0;
...
void LuaValue::Push(lua_State *lua) const
{
    ...
    case LUATYPE_LIGHTUSERDATA:
        lua_pushlightuserdata(lua, mLightUserData);
        break;
    case LUATYPE_USERDATA:
        lua_rawgeti(lua, LUA_REGISTRYINDEX, mUserData);
        break;
...
LuaValue LuaValue::Pop(lua_State *lua)
{
    ...
    case LUA_TBOOLEAN:
        ...
        break;
    case LUA_TUSERDATA:
        /* // I don't have access to global at this point, i'm going to stub this out for now - since I only need the push right now
        lua_pushvalue(lua, -1);
        lua_xmove(lua, global, 1);
        value.SetUserData(luaL_ref(global, LUA_REGISTRYINDEX));
        */
        break;
    case LUA_TSTRING:
        {
            size_t      size = 0;
            const char *text = lua_tolstring(lua, -1, &size);
            uint64_t    uint64 = 0;
            int64_t     int64 = 0;
            ...
            {
                value.SetString(::String(text, size));
            }
            lua_pop(lua, 1);
        }
        break;
    ...
    case LUA_TLIGHTUSERDATA:
        value.SetLightUserData(lua_touserdata(lua, -1));
        lua_pop(lua, 1);
        break;
    case LUA_TFUNCTION:
...
{
    ::String keyString(key.ToString(lua));
...
LuaTableIterator::LuaTableIterator(const LuaTable &table) : mTable(table), mHasNext(true), mCount(0), mTop(0)
{
    lua_State *state = mTable.GetState();
    mTop = lua_gettop(state);
...
LuaTableIterator::~LuaTableIterator()
{
    ...
    core_assert(mTop == lua_gettop(mTable.GetState()));
}
...
{
    Create(lua);
    std::map<const void *, String>::const_iterator i = mMap.find(function);
    if (i != mMap.end())
...
void LuaFunctionMap::PopulateFunctionMap(lua_State *lua, const LuaTable &table, std::map<const void *, String> &map)
{
