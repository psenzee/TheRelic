...
function SavePrimaryPlayer()
  print("SavePrimaryPlayer!")
  if GetPlayer():GetHitPoints() <= 0 then
    print("Not saving, player is dead!")
  else
    SaveGame(GetPlayer(), "saveprofile1.bin")
  end
end

function LoadPrimaryPlayer()
  print("LoadPrimaryPlayer!")
  LoadGame(GetPlayer(), "saveprofile1.bin")
end
...
function MultiplayerPlayerUpdate()
  ...
  for i = 1, 4 do
    ...
    local packet = CHARACTER_POSITION_PACKETS[i]
...
function UpdateGame(instance)
  ...
  if ButtonPressed(3) then
    ...
    --TestLowEndDevice()
  --[[
    print("about to print local game state")
    PrintTable(LOCAL_GAME_STATE)
    local data = Serialize(LOCAL_GAME_STATE["DIALOG_STACK"])
  ...
  -- $TODO we should NOT do this every frame!
