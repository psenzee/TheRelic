...
/*
EventDispatcher *GetGlobalEventDispatcher()   { return state->GetEventDispatcher(); }
EventDispatcher *GetGlobalUiEventDispatcher() { return state->GetUiEventDispatcher(); }
EventTimer      *GetGlobalEventTimer()        { return state->GetEventTimer(); }
*/

GameState::GameState(const core::Size &framesz) :
    mBackground(0), mCamera(0), mInput(0), mQuads(0), 
    mGameUi(0), mGame(0),
    mRenderables(0), mGameThread(0), mUiCore(0), mFrames(0), mOverlayColor(Vector4(1.f, 1.f, 1.f, 0.f))
{
...
{
    const char *pages[] = { "OptimusPrincepsSemiBold_S64B1I0.xml", 
                            "OptimusPrincepsSemiBold_S64B1I0.xml", 
                            "OptimusPrincepsSemiBold_S64B1I0.xml", 0 };
    mGameUi     = new GameUI(pages);
    mUiCore     = new UiCore(*mGameUi, *mQuads, mUiEventDispatcher);

    GraphicsDevice::CreateInstance(framesz);
...
void GameState::Draw(const GameTime &time)
{
    ...
    if (mOverlayColor.w >= 0.05f)
    {
        ...
        QuadRenderer::RenderScreenAlignedQuad(overlay, mOverlayColor,
                                              Vector3(-1000.f, -1000.f, -150.f), Vector3(1000.f, 1000.f, -150.f),
