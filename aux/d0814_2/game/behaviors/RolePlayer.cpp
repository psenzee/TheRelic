...
RolePlayer::RolePlayer() : AbstractBehavior(TYPE), mRandom(0), mStrength(0), mDexterity(0), mVitality(0), mEnergy(0), mLuck(0.f), mClassBonus(0.f), mRatingBonus(0.f)
{
}
...
float RolePlayer::GetAttackRating() const
{
    ...
    // = (Dex * 5) - 35 + ClassBonus 
    return (GetDexterity() * 5.f) - 35.f + GetClassBonus() + GetRatingBonus();
}
