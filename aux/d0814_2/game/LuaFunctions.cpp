...
{
    Vector2 v = state->GetInput().move.GetVector();    
...
static int SetVectorInput(lua_State *lua)
{
    ...
    state->GetInput().move.SetVector(Vector2((float)lua_tonumber(lua, -2), // x and y are swapped in the accel vector
...
static int SetVisitedData(lua_State *lua)
{
    ...
    level->SetVisitedData(data, size);
    return 0;
}
...
struct DeferredCall
{
    enum Deferred { LOAD_LEVEL, UNLOAD_LEVEL, RELOAD_LEVEL, DESTROY_CHARACTER, START_GAME_STATE, START_GAME, END_GAME };
...
static int CreateCloudIndependent(lua_State *lua)
{
    ...
    Vector3         position(GetVector3(lua, -3));
    DeviceTexture  *texture = ContentLoader::GetInstance()->GetTexture(GetString(lua, -6));
    ParticleSystem *ps = ParticleEffects::GetInstance()->CreateCloudIndependent(texture, position, time, speed, false, true);
    lua_pushlightuserdata(lua, ps);
    return 1;
}
...
static int CreatePerpetualCloudIndependent(lua_State *lua)
{
    ...
    Vector3         position(GetVector3(lua, -3));
    DeviceTexture  *texture = ContentLoader::GetInstance()->GetTexture(GetString(lua, -6));
    ParticleSystem *ps = ParticleEffects::GetInstance()->CreateCloudIndependent(texture, position, time, speed, true, false);
    lua_pushlightuserdata(lua, ps);
    return 1;
}
