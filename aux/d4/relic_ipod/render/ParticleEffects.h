...
public:
    ...
    typedef void (*ParticleFunction)(Particle &);
...
public:
    ...
    ParticleSystem         *CreateCloud (DeviceTexture *texture, const Vector3 &position, float time, float speed, bool perpetual, bool autodestroy);
    ParticleSystem         *CreateCloudIndependent(DeviceTexture *texture, const Vector3 &position, float time, float speed, bool perpetual, bool autodestroy);
...
{
    enum { QUADS = 256 };
    QuadType mQuads[QUADS];

public:

    RotatedQuads()
    {
        // $TODO INITIALIZE mQuads
    }

    inline QuadType GetQuad(float angle)
    {
