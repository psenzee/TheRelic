                     ...
                     const Material          &material)
{
    ...
    return new Drawable(renderName, mesh, transform, material, states, texture);
}
                          ...
                          const Material          &material)
{
    ...
    return new Drawable(renderName, imd, transform, material, states, texture);
}

Drawable::Drawable(const RenderSet::String &renderName, IDrawable *drawable, const Matrix4f &transform, const Material &material, const RenderStates &states, Texture *texture) :
...
void Drawable::RenderImmediate(RenderContext &context)
{
    if (mDrawable)
    {
        ...
        rc.transform = context.transform * mTransform;
        if (mTexture)
...
void Drawable::Render(RenderContext &context)
{
    if (mDrawable)
    {
        ...
        DrawItem item(rc.transform, mDrawable, mTexture, &mStates, &mMaterial, center.z);
