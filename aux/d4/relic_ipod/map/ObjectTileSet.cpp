...
ObjectTileSet::ObjectTileSet(XmlReadContext *context) : context(context)
{
...
std::vector<IGameObject *> ObjectTileSet::CreateGameObjects(XmlReadContext *context)
{
    ...
    printf("Found tiles: ");
    for (std::map<std::string, IGameObject *>::iterator i = context->objects.begin(), e = context->objects.end(); i != e; ++i)
    {
