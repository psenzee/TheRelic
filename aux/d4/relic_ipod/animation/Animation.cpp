...
Animation::Animation() : frames(0), frame(0), elapsed(0),
                         frameTimeMs(33), loop(true), ended(false), mTagCallback(0), mTagCallbackUser(0)
{
}
...
static void Draw(IGameObject *object, RenderContext &context, const Matrix &m)
{
    if (object)
    {
...
static void DrawFrame(IGameObject *object, int frame, RenderContext &context, const Matrix &m)
{
    if (object)
        Draw(object->GetObjectList().GetObjects()[frame], context, m);
}

void Animation::Render(RenderContext &context, const Matrix &transform)
{
    Draw(GetFrame(), context, transform);
}
...
int GetTagIntArgument(const char *data, const char *name)
{
    ...
    if (read(&data, value))
        return value;
    printf("Animation: expected number after '%s' tag!\n", name);
    return -1;
}
