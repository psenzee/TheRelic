...
void PackString(char **to, const std::string &str, const NetDictionary *dictionary)
{
    PackString(to, str.c_str(), str.size(), dictionary);
}

void PackString(char **to, const char *str, int strsize, const NetDictionary *dictionary)
{
    if (dictionary)
    {
...
int UnpackString(const char **from, char *str, int maxsize, const NetDictionary *dictionary)
{
    char c = **from;
    ...
    if (c & PACK_DICTIONARY_STRING)
    {
        ...
        strncpy(str, dictionary->GetText(index), maxsize);
        str[maxsize - 1] = '\0';
...
int UnpackString(const char **from, std::string &str, const NetDictionary *dictionary)
{
    enum { MAX_LOCAL_SIZE = 1024 };
    char buffer[MAX_LOCAL_SIZE];
    int size = UnpackString(from, buffer, MAX_LOCAL_SIZE, dictionary);
    if (size < 0)
        return -1;
