...
RollingBall::RollingBall(XmlElement *options, const char *name) : bounciness(1.0f / 3.0f), conservation(0.0f), services(0), ballDynamic(0), ballStatic(0), ballShadow(0), ballHalo(0), halofader(0.0f)
{
    /*
...
void RollingBall::RenderHalo(const OverheadCamera &camera)
{
    if (ballHalo)
    {
        ...
        ballHalo->Render(*(services->device), t, camera, true);
        services->device->EnableDepthTest(true);
    }
}
