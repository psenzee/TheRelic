...
#ifdef LITE_VERSION
        scores.SetFile(globalTranslateReadWritePath("scoreslite33.bin"));
#else
...
void GameState::Save()
{
    if (GetLevel()->GetLevelNumber() > LAST_LEVEL - 1)
...
#ifdef LITE_VERSION
static const int LEVEL_NORMALIZE_BALL_WEIGHT = 5;
#else
...
float GameState::GetBumpMagnitude()
{
    ...
    float start = 0.5f;
#endif        
    return _level_interpolate(start, 2.5f, GetLevel()->GetLevelNumber());
}
...
float GameState::GetBallVelocityScale()
{
    ...
    float start = 40.0f;
#endif    
    return _level_interpolate(start, 200.0f, GetLevel()->GetLevelNumber());    
}
...
void GameState::EndGame()
{
    ...
    SetPaused(true);    
    uicontroller.ShowText("\\#6600ffGame Complete!", text, UIController::EXIT_END_GAME);
#else
    static const char *text[] =
    {
        "\n\nYou've completed\n"
        "\\#6600ffIndigo Ocho Lite\\#ffffff!!\n\n",
        /*
        "Get the full version of\n"
        "\\#6600ffIndigo Ocho\\#ffffff\n"
        "for 35 action-packed levels\n",        
        
        "\nand a free maze mode\n"
        "with an unlimited number\n"
        "of levels that change each\n"
        "time you play!\n",
         */
    ...
    SetPaused(true);    
    uicontroller.ShowText("\\#6600ffCONGRATULATIONS!", text, UIController::EXIT_APPSTORE);    
#endif        
}
...
void GameState::ShowTrappedTutorial()
{
    ...
    SetPaused(true);    
    uicontroller.ShowText("\\#ff0000TRAPPED!", text, UIController::EXIT_GAME_OVER);
}
...
void GameState::Start(int level)
{
    ...
    StartNewLife();
#ifdef SHOW_INTRO_TUTORIAL
    if (GetLevel()->GetLevelNumber() == INITIAL_TUTORIAL_LEVEL)
    {
...
void GameState::StartNewLife()
{
...
#ifdef LITE_VERSION
    const int PAIR_DIVISOR =  5;
#else
    const int PAIR_DIVISOR = 10;
#endif

    bool showTutorials = false;
#ifdef SHOW_TUTORIALS
    showTutorials = true;
#endif
    
    if (mode == INDIGO_OCHO)
    {
        if (GetLevel()->GetLevelNumber() >= MUTUAL_DESTRUCTION_TUTORIAL_LEVEL)
        {
            int pairs = GetLevel()->GetLevelNumber() / PAIR_DIVISOR + 1;
            challenge = new MutualDestruction(this, pairs, showTutorials && GetLevel()->GetLevelNumber() == MUTUAL_DESTRUCTION_TUTORIAL_LEVEL);
        }
        else if (GetLevel()->GetLevelNumber() >= BUMP_TUTORIAL_LEVEL)
            challenge = new BumpTutorial(this, showTutorials && GetLevel()->GetLevelNumber() == BUMP_TUTORIAL_LEVEL);
    }
    ...
    if (mode == INDIGO_OCHO && GetLevel()->GetLevelNumber() >= LAST_LEVEL + 1)
    {
        EndGame();
    }

#ifdef LITE_VERSION
    const int ADDITIONAL_TIME = 30;
#else
    const int ADDITIONAL_TIME =  0;
#endif
    
    if (mode == INDIGO_OCHO)
        timeRemaining = (GetLevel()->GetOptions()->time + levels->GetLevelNumber() * 2 + ADDITIONAL_TIME) * 1000;
    else
        timeRemaining = 10000000; // we never run out in maze mode
}
...
#undef NARRATE_YELLOW    
    ...
    else if (pages[state])
    {
        ...
        float delta = (time - lastTime) * 0.025f;
        y += delta;
        if (fabsf(y) <= 2.0f)
        {
            // title fade out
...
void GameState::Event_OnPlayerKilled()
{
    reachedExit = false;
    //player->OnKilled();
    ui->SendBallMessage("\\#2800ffYou Died!");
}
...
void GameState::Event_OnExitReached()
{
    if (!reachedExit)
    {
        ...
        if (mode == INDIGO_OCHO && level < LAST_LEVEL - 1)
            save.AutoSave(tscore);
        ui->SendBallMessage((mode == INDIGO_OCHO && scores.IsLevelHighScore(lscore)) ? 
                            "\\#ffff00Level High Score!" : "\\#ffff00Level Complete");
        int rank = scores.GetHighScoreIndex(tscore);
        if (rank >= 0 && rank < 10)
            ui->SendBallMessage("\\#ffffffNew High Score!", 40);
        scoreContext.score.score = score;
        scoreContext.score.level = level;
#ifndef LITE_VERSION
        if (mode == INDIGO_OCHO && (level < LAST_LEVEL || strcmp(scoreContext.score.name, "<no name>") != 0)) // if they haven't entered a name til the end, they're out of luck, because the end of game stuff gets ruined by the name popup
...
void GameState::Event_OnTrapped()
{
    if (!timeExpired)
    {
        ui->SendBallMessage("\\#ff0000TRAPPED!");        
        if (!trappedTutorial)
            GameOver();
        else
        {
