...
Level::Level(GameState *state, const char *path, int level) : state(state), levelnumber(level), locked(false), exitstate(EXIT_VALID)
{
    ...
    int size = 0;
#ifdef LITE_VERSION    
    int liteMaps[] = { 0, 1, /*21,*/ 8, 9, 10, 21, 26, 27 };
    if (level >= sizeof(liteMaps) / sizeof(liteMaps[0]))
        level = 30;
    else
        level = liteMaps[level];
    if (level == 1)
        size++;
#endif
    size += 4 + int(level * EXPANSION);
    map = mg.GenerateMap(level, core::Size(size, size), options->configuration, "[random]", seed, levelnumber);    
}
...
void LevelSequence::Draw(const GameTime &time)
{
    ...
    */
    if (levels[currentIndex])
        levels[currentIndex]->Draw(time, *(state->camera));
}
