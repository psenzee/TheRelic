...
void GameState::StartNewLife()
{
    ...
    if (mode == INDIGO_OCHO)
    {
        ...
        {
            int pairs = GetLevel()->GetLevelNumber() / 5 + 1;
            challenge = new MutualDestruction(this, pairs, false/*GetLevel()->GetLevelNumber() == MUTUAL_DESTRUCTION_TUTORIAL_LEVEL*/);
        }
        else if (GetLevel()->GetLevelNumber() >= BUMP_TUTORIAL_LEVEL)
            challenge = new BumpTutorial(this, false/*GetLevel()->GetLevelNumber() == BUMP_TUTORIAL_LEVEL*/);
    }
    ...
    if (mode == INDIGO_OCHO)
        timeRemaining = (GetLevel()->GetOptions()->time + levels->GetLevelNumber() * 2 + 30) * 1000;
    else
