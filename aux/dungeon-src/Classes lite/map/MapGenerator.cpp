...
*/    
    ...
    if (level != 8 && strcmp(tilesets_common[tileset_common_index], "tilesets_superball.xml") == 0) // terrible hack
        tileset_common_index = (tileset_common_index + 1) % TILESET_COMMON_COUNT;
...
#if 0
Map *MapGenerator::GenerateMap(int level, const core::Size &mapsz, XmlElement *levelConfig, const char *mapTypeName)
{
    MapFile   options(levelConfig, mapTypeName);
    ITileMap *map     = 0;
    // generate level
    const char *tilesets[] = 
    { "tileset0x.xml",  "tileset2x.xml",  // gardens
        "tileset3x.xml",                    // glass loo
        "tileset5x.xml",  "tileset5x.xml",  // test tracks
        //   "tileset4x.xml",  "tileset16x.xml", // warehouses
        "tileset7x.xml",                    // the loo
    ...
    int tileset_index        = (level / /*8*/2) % TILESET_COUNT,
    tileset_common_index = (tileset_index / TILESET_COUNT) % TILESET_COMMON_COUNT;
    
    options.tiles = tilesets[tileset_index];
    ((CoreTileMap *)map)->Reverse();
    ITileMapModel *model = 
        new SimpleTileMapModel(DeviceTileSetLoader::GetTileSet(options.tiles, tilesets_common[tileset_index/* == 1 ? 1 : 0*/]/*tilesets_common[tileset_common_index]*/, State->services), map);
    
    return new Map(State->services, model, &options);
}
#endif
