...
{ 
    ...
    for (std::vector<MenuItem>::iterator i = items.begin(), e = items.end(); i != e; ++i)
        (*i).touched.Fade(0.0f, 1);
}

void Menu::Set(GameUI &ui)
{
    ...
    if (!title.empty())
    {
        ...
        ui.DrawStringCentered(s, Vector2(0.0f, 80.0f), alpha, 32.0f);
    }
    ...
    for (std::vector<MenuItem>::iterator i = items.begin(), e = items.end(); i != e; ++i)
    {
        ...
        if (!item.text.empty())
        {
            ...
            float sz = size, animated = item.touched.GetValue();
            if (animated < 1.0f)
                sz += sz * animated * 3.0f;
//            else
//                item.touched.Fade(0.0f, 500);
            ui.DrawStringCentered(s, at, alpha * (1.0f - animated), sz);
        }
        count++;
    }
}
...
void Leaderboard::WriteMainHighScoreLine(GameUI &ui, float start, float size, int index, int begin, float alpha)
{
    ...
    {    
        ...
        ui.DrawString(s, Vector2(150.0f, at), alpha, size);
        sprintf(s, "L%u", score.level);
        ui.DrawString(s, Vector2(-30.0f, at), alpha, size);        
        sprintf(s, "%u", score.score);
        ui.DrawString(s, Vector2(-90.0f, at), alpha, size);
    }
}
...
{
    if (s && *s)
        ui.DrawStringCentered(s, Vector2(0.0f, 100.0f), alpha, 32.0f);
}
...
void TextScreen::DrawText(GameUI &ui, const char *s, float start, float size, float alpha)
{
    ui.DrawTextCentered(s, Vector2(0.0f, start), alpha, size);
    /*
    if (s && *s)
    {
        char **split  = split_char_of(s, "\n");
        for (int i = 0; split[i]; i++)
            ui.DrawStringCentered(split[i], Vector2(0.0f, start - size * i), alpha/* * 0.5f* /, size);    
        destroy(split);
//      ui.DrawString(s, Vector2(150.0f, start), alpha, size);
    }
    */
}
...
void TextScreen::Set(GameUI &ui)
{
    ...
    if (!text.empty() && screen < (int)text.size())
        DrawText(ui, text[screen].c_str(), start, size, alpha);
    if (screen < (int)(text.size() - 1))
    {
        ui.DrawStringCentered("\\#808080>>", Vector2(0.0f, -110.0f), alpha, 32.0f);
        if (exitfn)
            ui.DrawString("\\%c0c0c060~", Vector2(-150.0f, -105.0f), alpha, 26.0f);
    }
}
