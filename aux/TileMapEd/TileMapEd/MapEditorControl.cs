...
namespace TileMapEd
{
    public partial class MapEditorControl : UserControl
    {
        ...
        private ITileMap GetTileMap()
        {
            return worldMapControl.GetTileMap();
        }

        private void SetTileMap(ITileMap map)
        {
            worldMapControl.SetTileMap(map);
        }
        ...
        private void LoadMapPane(string filename)
        {
            ...
            try
            {
                ...
                SetTileMap(LoadMap(mapFileName));
                RefreshMap();
            }
            catch (Exception)
            {
        ...
        {
            if (tileSet == null || GetTileMap() == null)
            {
                // error dialog here
            }
            else
            {
                worldMapControl.SetMap(new SimpleTileMapView(tileSet, GetTileMap()));
            }
        }
        ...
        private ITileMap mTileMap = null;
        private Random   random  = new Random();

        private void newMap_Click(object sender, EventArgs e)
        {
            SetTileMap(TileMaps.CreateMap(new Size(32, 32)));
            RefreshMap();
        }

        private void newTab_Click(object sender, EventArgs e)
        {
            worldMapControl.AddMapTab();
        }
        ...
        private void saveMap_Click(object sender, EventArgs e)
        {
            if (GetTileMap() != null)
            {
                ...
                if (mapField.Text.Trim().Length != 0)
                    GetTileMap().Write(saveMapDialog.FileName);
            }
        }
        ...
        private void contextMenuStrip1_Opening(object sender, CancelEventArgs e)
        {
            if (GetTileMap() != null)
            {
                ...
                selected = GetTileMap().GetTileId(p.X, p.Y);
            }
        }
        ...
        {
            SetTileMap(TileMaps.CreateMap(new Size(16, 16)));
            MazeGenerator mg = new MazeGenerator();
            mg.Create(GetTileMap(), 100);
            RefreshMap();
        }
        ...
        {
            MazeGenerator mg = new MazeGenerator();
            SetTileMap(mg.CreateFull1(new Size(16, 16), 100));
            RefreshMap();
        }
        ...
        {
            MazeGenerator mg = new MazeGenerator();
            SetTileMap(mg.CreateFull5(new Size(64, 64), random.Next()));
            RefreshMap();
        }
