...
RECEIVE_HIT_QUEUE = {}
RECEIVE_HIT_IN_MOTION_QUEUE = {}

function EnqueueReceiveHit(victim, aggressor, points, knockback)
  table.insert(RECEIVE_HIT_QUEUE, { victim, aggressor, points, knockback })
end

function EnqueueReceiveHitInMotion(victim, aggressor, points, knockback)
  table.insert(RECEIVE_HIT_IN_MOTION_QUEUE, { victim, aggressor, points, knockback })
end

function ProcessReceiveHitQueue()
  for i, v in ipairs(RECEIVE_HIT_QUEUE) do
    MultiplayerReceiveHit(v[1], v[2], v[3], v[4])
  end
  RECEIVE_HIT_QUEUE = {}
  for i, v in ipairs(RECEIVE_HIT_IN_MOTION_QUEUE) do
    MultiplayerReceiveHitInMotion(v[1], v[2], v[3], v[4])
  end
  RECEIVE_HIT_IN_MOTION_QUEUE = {}  
end
...
function AddKill(player)
  ...
  data.unspentKills = data.unspentKills + 1
  local kills = data.kills
  SetAchievementValue('Kills', data.kills)
end

function GetTargetPlayer(c)
  if not IsMultiplayer() or not c then
    return GetPlayer()
  end
  local player = GetActivePlayer(c:GetSeed() % GetActivePlayerCount())
  if player == nil then
    return GetClosestPlayer(player)
  end
  return player
end
...
function SetTarget(c)
  if LOCAL_GAME_STATE.TurnEnemies then
    ...
    local player = GetTargetPlayer(c)
    c:SetApproachTarget(player)
    c:SetAttackTarget(player)
  end
end
...
dofile (MakeReadOnlyPath("projectile.lua"))
dofile (MakeReadOnlyPath("playerprojectile.lua"))
...
local ENEMIES = { { weight = 0.2,  factory = CreateSkeleton,    type = "Skeleton0",    hp = 250,  speed = 4.0 },
                  ...
                  { weight = 0.05, factory = CreateNecromancer, type = "Necromancer0", hp = 600,  speed = 6.0 },
                  { weight = 0.05, factory = CreateSpider,      type = "Spider0",      hp = 500,  speed = 2.0 },
                  { weight = 0.05, factory = CreateNecromancer, type = "Necromancer1", hp = 750,  speed = 8.0 },
...
local ENEMIES_DIFFICULT
              = {--{ weight = 0.25, factory = CreateSkeleton,    type = "Skeleton6",    hp = 1000, speed = 4.0 },
                 --{ weight = 0.25, factory = CreateSkeleton,    type = "Skeleton7",    hp = 1000, speed = 5.0 },
                   { weight = 0.50, factory = CreateNecromancer, type = "Necromancer1", hp = 1000, speed = 6.0 },
                   { weight = 0.50, factory = CreateSpider,      type = "Spider4",      hp =  750, speed = 3.0 },
                   }                  

--[[
local ENEMIES = { { weight = 0.2,  factory = CreateSkeleton,    type = "Skeleton0",    hp = 250,  speed = 3.4 },
                  { weight = 0.6,  factory = CreateSkeleton,    type = "Skeleton1",    hp = 100,  speed = 3.5 },
                  { weight = 0.05, factory = CreateNecromancer, type = "Necromancer0", hp = 600,  speed = 4.3 },
                  { weight = 0.05, factory = CreateSpider,      type = "Spider0",      hp = 500,  speed = 3.3 },
                  { weight = 0.05, factory = CreateNecromancer, type = "Necromancer1", hp = 750,  speed = 4.1 },
                  { weight = 0.05, factory = CreateSpider,      type = "Spider1",      hp = 500,  speed = 3.5 },
                  }
                  
local ENEMIES_DIFFICULT
              = {--{ weight = 0.25, factory = CreateSkeleton,    type = "Skeleton6",    hp = 1000, speed = 4.0 },
                 --{ weight = 0.25, factory = CreateSkeleton,    type = "Skeleton7",    hp = 1000, speed = 5.0 },
                   { weight = 0.50, factory = CreateNecromancer, type = "Necromancer1", hp = 1000, speed = 3.2 },
                   { weight = 0.50, factory = CreateSpider,      type = "Spider4",      hp =  750, speed = 3.1 },
                   }  
]]--

function CreateAndSerialize(factory, type, hp, speed, x, y, z)
  local created = factory(type, hp, speed, x, y, z, MakeRandomSeed())
  if created == nil then
