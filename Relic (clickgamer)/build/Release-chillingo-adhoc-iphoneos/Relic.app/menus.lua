...
function CreateMenus()
  ...
  BACKGROUND_MENU_IMAGE = Ui_CreateImageButton("Background1", "intro10c_map_256sq", "", 0, bgtop, 480, bgbottom, 1.0, 18.75, 0, 1, 1, 0, 0)
  BACKGROUND_MAIN_MENU_IMAGE = Ui_CreateImageButton("BackgroundMain", "menuback_partial", "", 0, bgtop, 480, bgbottom, 1.0, 18.75, 0, 1, 0.9375, 0.375, 0)  
...
function CreateMainMenu()
  ...
  AddImageMenuButton(MAIN_MENU, "MainMenu_Crystal", "crystal", "Online - Crystal", width, 0.32, "CRYSTAL_BUTTON") 
...
function UpdateCrystalState()
  local button = _G["CRYSTAL_BUTTON"]
  if GetGameStateId() == GAMESTATE_MENU and button ~= nil then
    local active = IsCrystalActive()
    if CRYSTAL_LAST_ACTIVE_STATE ~= active then
	  CRYSTAL_LAST_ACTIVE_STATE = active
	  CrystalChangedState(CRYSTAL_LAST_ACTIVE_STATE)
	  if active ~= false then
	    UiControl_SetText(button, "\\#808080Online - Crystal")
	  else
	    UiControl_SetText(button, "Online - Crystal")
	  end
	  if UiControl_IsVisible(button) then
	    _SetVisible(button, false)
	    _SetVisible(button, true)	  
	  end
      UiControl_SetActive(button, not active)
	end
  end
end
...
--AddImageMenuButton(EXTRAS_MENU, "CrystalAchievements", "crystal", "Achievements", width, 0.35)
--AddImageMenuButton(EXTRAS_MENU, "CrystalLeaderboards", "crystal", "Leaderboards", width, 0.615)
...
--AddImageMenuButton(SAVE_EXIT_MENU, "CrystalAchievements", "crystal", "Achievements", width, 0.32)
--AddImageMenuButton(SAVE_EXIT_MENU, "CrystalLeaderboards", "crystal", "Leaderboards", width, 0.68)  
  AddMenuButton(SAVE_EXIT_MENU, "CrystalAchievements", "Achievements", width)
  AddMenuButton(SAVE_EXIT_MENU, "CrystalLeaderboards", "Leaderboards", width)
