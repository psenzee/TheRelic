...
function IsAlive(c)
  return c ~= nil and not c:IsDestroyed()
end
...
function ReceiveHitInMotion_ById(otherId, cId, points, kb)
  local other = GetCharacterById(otherId)
  local c     = GetCharacterById(cId)
  if other ~= nil and c ~= nil then
    other:ReceiveHitInMotion(c, points, kb)
  end
end
...
function MultiplayerReceiveHitInMotion(other, c, points, kb)
  other:ReceiveHitInMotion(c, points, kb)
  if IS_MULTIPLAYER then
    ReceiveHitInMotion_ById(other:GetId(), c:GetId(), points, kb)
  end
end
...
function HitCircleInRadius(c, radius, knockBack, points)
   ...
   for i = 1, #list do
     local other = list[i]
     if other:GetType() == 'Barrel' or (other:GetTypeId() == CONSTRAINED_COUNT_ENEMY_TYPE and IsAlive(other)) then
       local kb = other:data().knockBack
       if kb == nil then kb = 10 end     
       if diminish then
         local dist = CharacterDistance(c, other)
         if dist < radius then
           local ratio = 1.0 - (dist / radius)
           MultiplayerReceiveHit(other, c, points / ratio, kb)
         end
       else
         MultiplayerReceiveHit(other, c, points, kb)
       end
...
function UpdateImmolationStatus(c)
   local data = c:data()
   if data.immolate ~= nil then
     EnqueueReceiveHit(c, data.immolator, data.immolate, 5.0)
   end
end

function Immolate(c, immolator, points)
  if c:GetTypeId() == CONSTRAINED_COUNT_ENEMY_TYPE and IsAlive(c) then
    local x, y, z = c:GetPosition()
    local data = c:data()
    c:AddParticleEffect(PerpetualFireIndependent(x, y), -128)
    if data.immolate ~= nil then
      data.immolate = data.immolate + points
    else
      data.immolate = points
    end
    data.immolator = immolator
  end  
end

function ImmolateCircleInRadius(c, radius, knockBack, points)
   local list = GetAllCharactersInRadius(radius)   
   if c == nil then
     c = GetPlayer()
   end
   for i = 1, #list do
     if CharacterDistance(list[i], c) < radius then
       Immolate(list[i], c, points)
     end
   end
end
...
function MultiplayerAreaAttack(c)
  ...
  HitCircleInRadius(c, 128.0, 10.0, 1.0)
end