...
if value.s_seed == nil then value.s_seed = MakeRandomSeed() end
  local c = CreateBarrel(value.s_type, value.s_hp, value.s_speed, value.s_x, value.s_y, value.s_seed)
--c:SetId(value.s_id)    
...
function CreateBarrel_Character(c, type, hp, speed, x, y, seed)
   ...
   SetAttributes(c, RandomBetweenSeeded(seed, hp / 2, hp), Random_NextReal(seed) * speed)
   if Random_NextReal(seed) < 0.8 then
...
function CreateBarrel(type, hp, speed, x, y, seed)
   local c = NewCharacter(-1, type, type, x, y, 0.0)
   return CreateBarrel_Character(c, type, hp, speed, x, y, seed)
end