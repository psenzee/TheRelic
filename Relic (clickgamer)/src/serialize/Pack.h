...
const float ANGLE_FACTOR    = 128.0f / math::PIf;
...
inline AbstractPackType GetAbstractPackType(PackType type)
{
    ...
    case PACK_FUNCTION:   return ABSTRACT_PACK_FUNCTION;
    case PACK_BOOL_TRUE: 
...
inline bool IsPackTypeFunction(char c) { return c == PACK_FUNCTION; }
...
void PackString(char **to, const char *str, int size, const Dictionary *dictionary = 0);
void PackString(char **to, const std::string &str, const Dictionary *dictionary = 0);
int  GetUnpackStringSize(const char *from, const Dictionary *dictionary);
int  UnpackString(const char **from, char *str, int maxsize, const Dictionary *dictionary = 0);
int  UnpackString(const char **from, std::string &str, const Dictionary *dictionary = 0);
void PackNumber(char **to, float number);
template <typename T>
void PackNumber(char **to, T number) { PackNumber(to, static_cast<float>(number)); }
