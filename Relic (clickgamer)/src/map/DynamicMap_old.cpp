...
bool DynamicMap::Uncollide(item_t *item, const core::Point &at)
{
    ...
    float r = item->mRadius;
    Vector3 p(item->GetPosition());
    bool itemMovable = item->IsMovable();
    while (list)
    {
        if (list != item && list->IsCollidable())
        {
            ...
            if (dpq <= rr * rr)
            {
                if (dpq != 0.f)
                {
                    ...
                    bool listMovable = list->IsMovable();
                    if ((!listMovable && itemMovable) || list->IsFixed())
                        item->SetPositionRaw(p + d);
                    else if (listMovable && !itemMovable)
                        list->SetPositionRaw(q - d);
                    else
                    {
...
//                  continue;
                if (a->IsFixed() && b->IsFixed()) // if neither are movable, there's nothing we could do anyway
                    continue;
                ...
                if (dSq <= r * r)
                {
                    ...
                    if ((aIsMovable && bIsMovable) || (!aIsMovable && !bIsMovable && !a->IsFixed() && !b->IsFixed())) // split the difference, could be weighted
                    {                        
...
bool DynamicMap::Collides(const item_t *item) const
{
    ...
    /*
    core::Point p(Convert(item->mMapPosition));
    return Collides(item, p)                             ||
           Collides(item, core::Point(p.x - 1, p.y))     ||
           Collides(item, core::Point(p.x + 1, p.y))     ||
           Collides(item, core::Point(p.x, p.y - 1))     ||
           Collides(item, core::Point(p.x, p.y + 1))     ||
           Collides(item, core::Point(p.x - 1, p.y + 1)) ||
           Collides(item, core::Point(p.x + 1, p.y + 1)) ||
           Collides(item, core::Point(p.x - 1, p.y - 1)) ||
           Collides(item, core::Point(p.x + 1, p.y - 1));   // check home tile and surrounding tiles
    */
}
...
bool DynamicMap::Uncollide(item_t *item)
{
    ...
    for (int j = p.y, jsz = p.y + r.height; j < jsz; j++)
        for (int i = p.x, isz = p.x + r.width; i < isz; i++)
            if (Uncollide(item, core::Point(i, j)))
                return true;
    return false;
    */
    ...
    core::Point p(Convert(item->mMapPosition));
    return Uncollide(item, p)                             ||
           Uncollide(item, core::Point(p.x - 1, p.y))     ||
           Uncollide(item, core::Point(p.x + 1, p.y))     ||
           Uncollide(item, core::Point(p.x, p.y - 1))     ||
           Uncollide(item, core::Point(p.x, p.y + 1))     ||
           Uncollide(item, core::Point(p.x - 1, p.y + 1)) ||
           Uncollide(item, core::Point(p.x + 1, p.y + 1)) ||
           Uncollide(item, core::Point(p.x - 1, p.y - 1)) ||
           Uncollide(item, core::Point(p.x + 1, p.y - 1));   // check home tile and surrounding tiles
    */
}
