...
public:
    ...
    inline explicit Movable(const Vector3 &position) : mPositionVelocity(position), mMapPosition(Vector2(position.x, position.y)), mNext(0), mPrev(0), mMap(0), mRadius(sDefaultRadius), mId(-1), mType(0) {}
    inline explicit Movable(const PositionVelocity &pv) : mPositionVelocity(pv), mMapPosition(Vector2(pv.GetPosition().x, pv.GetPosition().y)), mNext(0), mPrev(0), mMap(0), mRadius(sDefaultRadius), mId(-1), mType(0) {}
    virtual ~Movable() {}
    ...
    virtual int             GetTypeId()                          const { return mType; }
    virtual void            SetTypeId(int type)                        { mType = type; }
