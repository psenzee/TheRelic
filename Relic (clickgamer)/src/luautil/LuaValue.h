...
public:
    ...
    enum Type { LUATYPE_NONE, LUATYPE_BOOL, LUATYPE_INT64, LUATYPE_UINT64,
                LUATYPE_NUMBER, LUATYPE_STRING, LUATYPE_TABLE, LUATYPE_FUNCTION, LUATYPE_USERDATA, LUATYPE_LIGHTUSERDATA };
    ...
    void                  SetLightUserData(void *value);
    ...
    // Thread types could easily be added, if desired
    
    String                ToString(lua_State *lua) const;
    ...
    inline static LuaValue _String(const char *value)           { LuaValue lua; lua.SetString(value);        return lua; }
    inline static LuaValue _String(const ::String &value)       { LuaValue lua; lua.SetString(value);        return lua; }
    inline static LuaValue Function(const char *value)         { LuaValue lua; lua.SetFunction(value);      return lua; }
    inline static LuaValue UserData(int value)                 { LuaValue lua; lua.SetUserData(value);      return lua; }
    inline static LuaValue LightUserData(void *value)          { LuaValue lua; lua.SetLightUserData(value); return lua; }
...
private:
    ...
    std::map<const void *, String> mMap;

    // ok, we're going to have to prepend the table key from the previous pass here
    // for now, let's do global functions only
    static void PopulateFunctionMap(lua_State *lua, const LuaTable &table, std::map<const void *, String> &map);
};
