...
UiCore::UiCore(GameUI &gameUi, QuadRenderer &quads, EventDispatcher &dispatcher) : mGameUi(gameUi), mQuadRenderer(quads),
    mDrawLeftTop(240.f, 160.f), mDrawRightBottom(-240.f, -160.f), mScreenSize(480.f, 320.f), mDispatcher(dispatcher), mRootControl(0), mRenderables(0)
{
                          ...
                          const Vector2 &start, const Vector2 &end, float angle)
{
    ...
    mQuadRenderer.RenderScreenAlignedPanoramaRotate(
            texture, color,
            v3(ScreenToDrawPosition(start)),
            v3(ScreenToDrawPosition(end)), angle);
}
