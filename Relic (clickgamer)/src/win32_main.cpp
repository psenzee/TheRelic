...
float frameSizeMultiplier = 2.0f;
...
bool _globalMouseButtonPressed = false;
int  _globalMouseX = 0, _globalMouseY = 0;
...
extern "C" void StartWiFiPicker()
{
    ...
    int port = 70;
    LuaValue values[] = {  LuaValue::_String(address), LuaValue::Int(70) };
...
extern "C" const char *GetPlatformUniqueIdentifier() { return "0000000000000000"; }
...
{ 
    static char name[64];
    int out = 0;
    ::GetWindowThreadProcessId(g_hWnd, (LPDWORD)&out);
    sprintf(name, "Win32_%04x", out);
    return name;
}

extern "C" const char *GetPlatformSystemName()       { return "Win32 OS"; }
extern "C" const char *GetPlatformSystemVersion()    { return "1.0.0"; }
extern "C" const char *GetPlatformModel()            { return "Win32 PC"; }
extern "C" const char *GetPlatformMachine()          { return "Win32 PC"; }
extern "C" const char *GetPlatformLocalizedModel()   { return "Win32 PC"; }
extern "C" bool        GetPlatformIsFirstGen()       { return false; }
extern "C" bool        GetPlatformIsSecondGen()      { return false; }

void SetScreenTouch(int x, int y)
{
    y = frameSize.height - y;
    std::swap(x, y);
    _globalTouchPoint = Vector2(float(x), float(y));
    if (state)
        state->GetInput().ScreenTouchBegan(0, core::Point(x, y));
    _globalMouseButtonPressed = true;
    _globalMouseX = x; _globalMouseY = y;
}

bool IsMousePressed()
{
   return _globalMouseButtonPressed;
}

void SetScreenTouchUpdate(int x, int y)
{
    if (IsMousePressed())
    {
        y = frameSize.height - y;
        ...
        if (state)
        {
            if (x == _globalMouseX && y == _globalMouseY)
                state->GetInput().ScreenTouchStationary(0, core::Point(x, y));
            else
                state->GetInput().ScreenTouchMoved(0, core::Point(x, y));
            _globalMouseX = x; _globalMouseY = y;
        }
    }
}

void SetScreenTouchEnd(int x, int y)
{
    // convert to iphone like coors
    /*
    core::Point p(static_cast<int>(x * .65f), static_cast<int>(y * 1.5f));
    _globalTouchPoint = Vector2(float(p.x), float(p.y));
    if (state)
        state->input->ScreenTouchBegan(0, core::Point(p.x, p.y));..
    */
    //x = frameSize.width - x/;
    y = frameSize.height - y;
    std::swap(x, y);
    _globalTouchPoint = Vector2(float(x), float(y));
    if (state)
        state->GetInput().ScreenTouchEnded(0, core::Point(x, y));
    _globalMouseButtonPressed = false;
    _globalMouseX = x; _globalMouseY = y;
}
...
//	state->GetCamera()->SetTilt(state->input->move.GetVector() * _g_tiltFactor);
...
void Update(GameState *state)
{
	...
	state->GetCamera()->SetTilt(Vector2(0.f/*200.0f*/, 400.0f));
//	state->GetCamera()->SetTilt(Vector2(0.f/*200.0f*/, 0.0f));
...
void Setup()
{
    ...
    glViewport(0, 0, (int)(frameSize.width * frameSizeMultiplier), (int)(frameSize.height * frameSizeMultiplier));
                   ...
                   LPSTR lpCmdLine, int iCmdShow)
{
    ...
    hWnd = CreateWindow( 
        ...
        0, 0, (int)(frameSize.width * frameSizeMultiplier) + frameEdges.width, (int)(frameSize.height * frameSizeMultiplier) + frameEdges.height,
...
extern void write_png_file(const char *file_name, int width, int height, const int *data);

extern "C" void CaptureScreen()
{
    int WIDTH = (int)(frameSize.width * frameSizeMultiplier), HEIGHT = (int)(frameSize.height * frameSizeMultiplier);
    enum { /*WIDTH = 480, HEIGHT = 320, */BYPP = 4, FNAME_MAX = 256 };
    static int sscount = 0;
    int *data = new int [WIDTH * HEIGHT];
    memset(data, 0, WIDTH * BYPP * HEIGHT);
    glReadPixels(0, 0, WIDTH, HEIGHT, GL_RGBA, GL_UNSIGNED_BYTE, data);
    char fname[FNAME_MAX];
    Flip((unsigned char *)data, WIDTH, HEIGHT, BYPP);
    //sprintf(fname, "ss%d.png", sscount++);
    sprintf(fname, "ss%d.png", timeGetTime());
    for (int i = 0, sz = WIDTH * HEIGHT * BYPP; i < sz; i += 4) // clear out alpha
        ((unsigned char *)data)[i + 3] = 255;
    write_png_file(fname, WIDTH, HEIGHT, data);
    delete [] data;
}
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_MOUSEMOVE:
        SetScreenTouchUpdate((int)(LOWORD(lParam) / frameSizeMultiplier), (int)(HIWORD(lParam) / frameSizeMultiplier));
        return 0;

    case WM_LBUTTONDOWN:
        printf("Mouse click at %d, %d\n", (int)(LOWORD(lParam) / frameSizeMultiplier), (int)(HIWORD(lParam) / frameSizeMultiplier));
        SetScreenTouch((int)(LOWORD(lParam) / frameSizeMultiplier), (int)(HIWORD(lParam) / frameSizeMultiplier));
        return 0;

    case WM_LBUTTONUP:
        printf("Mouse release at %d, %d\n", LOWORD(lParam) / frameSizeMultiplier, HIWORD(lParam) / frameSizeMultiplier);
        SetScreenTouchEnd(LOWORD(lParam) / frameSizeMultiplier, HIWORD(lParam) / frameSizeMultiplier);
        return 0;
...
/*
        case VK_SPACE:
            accel[0] = 0.f;
            accel[1] = 0.f;
            GetGlobalGame()->GetPlayer()->SetVelocity(Vector3(0.f, 0.f, 0.f));
            return 0;
        case VK_NEXT:
            if (state && state->GetCamera())
                state->GetCamera()->SetDepthScale(state->GetCamera()->GetDepthScale() + 0.1f);
            return 0;
        case VK_PRIOR:
            if (state && state->GetCamera())
                state->GetCamera()->SetDepthScale(state->GetCamera()->GetDepthScale() - 0.1f);
