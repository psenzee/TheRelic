...
NetGameServer::NetGameServer(UdpConnection *connection) : mConnection(connection), mMessage(0), mPlayerCount(1), mReceiveCallback(0), mReceiveUserData(0)
{
    mPlayers[HOST].address   = mConnection->GetAddress();
...
void NetGameServer::SendError(const Address &to, NetError::Error error)
{
    ...
    mConnection->SendTo(to, (const char *)&response, sizeof(response));
}
...
printf("** Sending JOIN_ACCEPT!\n");
    if (!mConnection->SendTo(addr, (const char *)&message, sizeof(NetGameMessage_JoinAccept)))
    {
...
bool NetGameServer::ProcessOne()
{
    ...
    ProcessTimeouts();
    Address from;
    int messageSize = mConnection->ReceiveFrom(from, mMessage, MAX_MESSAGE_SIZE);
    if (messageSize <= 0)
        return false;
...
{
    return mConnection->SendTo(mPlayers[player].address, data, length);
}
...
bool NetGameServer::SendGameData(int player, const char *data, int length)
{
    ...
    return mConnection->SendTo(mPlayers[player].address, mMessage, size + sizeof(NetGameMessageId));
}
...
int NetGameServer::ReceiveFrom(int *player, char *data, int length)
{
    Address addr;
    *player = -1;
    int r = mConnection->ReceiveFrom(addr, data, length);
    if (!r)
        return 0;
