...
NetGameClient::NetGameClient(UdpConnection *connection) : mConnection(connection), mMessage(0), mId(-1), mComplete(0), mUser(0), mTimestamp(0), mPingSent(0), mReceiveCallback(0), mReceiveUserData(0)
{
...
void NetGameClient::Join(const Address &host, complete_fn_t complete, void *user)
{
    ...
    if (!mConnection->SendTo(mHost, (const char *)&m, sizeof(m)))
...
{
    return mConnection->SendTo(mHost, data, length);
}

int NetGameClient::Receive(char *data, int length)
{
    Address host;
    int sz = mConnection->ReceiveFrom(host, data, length);
...
void NetGameClient::SendError(NetError::Error error)
{
    ...
    mConnection->SendTo(mHost, (const char *)&response, sizeof(response));
}
...
void NetGameClient::SendResponse(NetGameMessage::Type type)
{
    ...
    response.type     = type;
    mConnection->SendTo(mHost, (const char *)&response, sizeof(NetGameMessageId));
}
...
void NetGameClient::SendGameData(const char *data, int length)
{
    ...
    mConnection->SendTo(mHost, mMessage, size + sizeof(NetGameMessageId));
}
