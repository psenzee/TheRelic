...
public:
    ...
    inline int     GetLevelNumber() const { core_assert(mIndex >= 0); return mIndex; }
    ...
    inline Map           *GetMap(int map = Level::BASE_MAP)                         { return !mLevel ? 0 : mLevel->GetMap(map); }
    inline DynamicMap    *GetDynamicMap()                                           { return !mLevel ? 0 : mLevel->GetDynamicMap(); }
    inline ITile         *GetTileAt(int map, const Vector3 &position)               { return !mLevel ? 0 : mLevel->GetTileAt(map, position); }
    inline ITile         *GetTileAt(const Vector3 &position)                        { return GetTileAt(Level::BASE_MAP, position); }
    inline int            GetTileIdAt(int map, const Vector3 &position) const       { return !mLevel ? 0 : mLevel->GetTileIdAt(map, position); }
    inline int            GetTileIdAt(const Vector3 &position) const                { return GetTileIdAt(Level::BASE_MAP, position); }
    inline core::Point    GetTileLocationAt(int map, const Vector3 &position) const { return !mLevel ? core::Point() : GetTileLocationAt(map, position); }
