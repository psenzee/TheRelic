...
public:
    ...
    */
	void getLineList(int fromid, int toid, std::vector<int> &newlist) const
	{
	    bool addp  = false;
		int  point = 0;
	    for (size_t i = 0, sz = list.size(); i < sz || addp; i++)
		{
...
private:
    ...
    */
	static inline int m_point(const std::vector<int> &list, size_t index, int offset)
	{
	    size_t sz = list.size();
	    while ((int)index + offset < 0)
		    index += sz;
